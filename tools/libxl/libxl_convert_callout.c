...
int libxl__convert_legacy_stream(libxl__egc *egc,
	...
	if (!pid) {
		char * const args[] = { getenv("LIBXL_CONVERT_HELPER") ? :
		LIBEXEC_BIN "/convert-legacy-stream", "--in", GCSPRINTF("%d",
				chs->legacy_fd), "--out", GCSPRINTF("%d", fds[1]),
		/*
		...
		"--guest", chs->hvm ? "hvm" : "pv", "--format", "libxl",
		/* "--verbose", */
		NULL, };
		...
		libxl__exec(gc, -1, -1, -1, args[0], args, NULL);
	}
	...
	err: libxl__ao_abortable_deregister(&chs->abrt);
	assert(rc);
	return rc;
}
...
static void helper_exited(libxl__egc *egc, libxl__ev_child *ch, pid_t pid,
	...
	if (status) {
		...
		CTX, chs->rc ? XTL_DEBUG : XTL_ERROR, "conversion helper", pid, status);
