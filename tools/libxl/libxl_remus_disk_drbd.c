...
/* callbacks */
static void match_async_exec_cb(libxl__egc *egc, libxl__async_exec_state *aes,
		int rc, int status);
...
/* callbacks */
static void checkpoint_async_call_done(libxl__egc *egc, libxl__ev_child *child,
...
static void drbd_preresume(libxl__egc *egc, libxl__checkpoint_device *dev) {
	ASYNC_CALL(egc, dev->cds->ao, &dev->aodev.child, dev, drbd_preresume_async,
...
const libxl__checkpoint_device_instance_ops remus_device_drbd_disk = { .kind =
		LIBXL__DEVICE_KIND_VBD, .setup = drbd_setup, .teardown = drbd_teardown,
		.postsuspend = drbd_postsuspend, .preresume = drbd_preresume, };
