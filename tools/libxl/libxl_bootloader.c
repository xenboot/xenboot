...
static int parse_bootloader_result(libxl__egc *egc, libxl__bootloader_state *bl) {
	STATE_AO_GC(bl->ao);
	char buf[PATH_MAX * 2];
	FILE *f = 0;
	int rc = ERROR_FAIL;
...
static void bootloader_cleanup(libxl__egc *egc, libxl__bootloader_state *bl) {
	...
	if (bl->outputpath)
		libxl__remove_file(gc, bl->outputpath);
	if (bl->outputdir)
		libxl__remove_directory(gc, bl->outputdir);
...
static void bootloader_local_detached_cb(libxl__egc *egc,
		libxl__disk_local_state *dls, int rc);
...
static void bootloader_stop(libxl__egc *egc, libxl__bootloader_state *bl,
		int rc) {
	STATE_AO_GC(bl->ao);
	int r;
	...
	if (libxl__ev_child_inuse(&bl->child)) {
		...
		if (r)
			LOGE(WARN, "%sfailed to kill bootloader [%lu]",
...
void libxl__bootloader_run(libxl__egc *egc, libxl__bootloader_state *bl) {
	...
	log_time("Enter libxl__bootloader_init");
	...
	log_time("Enter bootloader_setpaths");
	...
	log_time("Enter libxl__device_disk_local_init");
...
static void bootloader_disk_attached_cb(libxl__egc *egc,
	...
	if (bootloader[0] != '/') {
		...
		if (lstat(bltmp, &st))
			LOG(DEBUG, "%s doesn't exist, falling back to config path", bltmp);
		else
			bootloader = bltmp;
	}
...
static void bootloader_gotptys(libxl__egc *egc, libxl__openpty_state *op) {
	...
	rc = setup_xenconsoled_pty(egc, bl, &dom_console_slave_tty_path[0],
	...
	bl->keystrokes.copywhat = GCSPRINTF("bootloader input for domain %"PRIu32,
			bl->domid);
	...
	bl->display.copywhat = GCSPRINTF("bootloader output for domain %"PRIu32,
			bl->domid);
	...
	for (const char **blarg = bl->args; *blarg; blarg++)
...
static void bootloader_finished(libxl__egc *egc, libxl__ev_child *child,
	...
	if (status) {
		...
		libxl_report_child_exitstatus(CTX, XTL_ERROR, "bootloader", pid,
				status);
		rc = ERROR_FAIL;
		goto out;
	} else {
