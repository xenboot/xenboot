...
struct libxl__osevent_hook_nexus {
	void *ev;
	void *for_app_reg;
	LIBXL_SLIST_ENTRY(libxl__osevent_hook_nexus)
	next;
};
...
static void osevent_release_nexus(libxl__gc *gc,
		libxl__osevent_hook_nexi *nexi_idle, libxl__osevent_hook_nexus *nexus) {
	nexus->ev = 0;
...
int libxl__ev_fd_register(libxl__gc *gc, libxl__ev_fd *ev,
		libxl__ev_fd_callback *func, int fd, short events) {
	int rc;
	...
	rc = OSEVENT_HOOK(fd, register, alloc, fd, &ev->nexus->for_app_reg, events,
			ev->nexus);
	if (rc)
		goto out;
...
static int time_rel_to_abs(libxl__gc *gc, int ms, struct timeval *abs_out) {
	int rc;
	struct timeval additional = { .tv_sec = ms / 1000, .tv_usec = (ms % 1000)
			* 1000 };
	struct timeval now;
...
static void time_deregister(libxl__gc *gc, libxl__ev_time *ev) {
	...
	if (!ev->infinite) {
		...
		OSEVENT_HOOK_VOID(timeout, modify,
				noop /* release nexus in _occurred_ */,&ev->nexus->for_app_reg,
				right_away);
		LIBXL_TAILQ_REMOVE(&CTX->etimes, ev, entry);
	}
}

static void time_done_debug(libxl__gc *gc, const char *func, libxl__ev_time *ev,
		int rc) {
#ifdef DEBUG
...
int libxl__ev_time_register_abs(libxl__ao *ao, libxl__ev_time *ev,
		libxl__ev_time_callback *func, struct timeval absolute) {
	AO_GC;
	int rc;

	CTX_LOCK;

	DBG("ev_time=%p register abs=%lu.%06lu", ev, (unsigned long)absolute.tv_sec,
			(unsigned long)absolute.tv_usec);
	...
	rc = 0;
	out: libxl__ao_abortable_deregister(&ev->abrt);
...
int libxl__ev_time_register_rel(libxl__ao *ao, libxl__ev_time *ev,
		libxl__ev_time_callback *func, int milliseconds /* as for poll(2) */) {
	AO_GC;
	struct timeval absolute;
	int rc;
	...
	out: if (!libxl__ev_time_isregistered(ev))
		libxl__ao_abortable_deregister(&ev->abrt);
	time_done_debug(gc, __func__, ev, rc);
	CTX_UNLOCK;
	return rc;
...
void libxl__ev_time_deregister(libxl__gc *gc, libxl__ev_time *ev) {
	...
	out: time_done_debug(gc, __func__, ev, 0);
	CTX_UNLOCK;
	return;
}

static void time_occurs(libxl__egc *egc, libxl__ev_time *etime, int rc) {
	DBG("ev_time=%p occurs abs=%lu.%06lu", etime,
			(unsigned long)etime->abs.tv_sec,
...
libxl__ev_xswatch *libxl__watch_slot_contents(libxl__gc *gc, int slotnum) {
	...
	if (slotcontents == NULL
			|| ((uintptr_t) slotcontents >= (uintptr_t) CTX->watch_slots
					&& (uintptr_t) slotcontents
							< (uintptr_t) (CTX->watch_slots +
							CTX->watch_nslots)))
...
static void watchfd_callback(libxl__egc *egc, libxl__ev_fd *ev, int fd,
	...
	for (;;) {
		...
		if (rc != 2) {
			LOG(ERROR, "watch epath=%s token=%s: failed to parse token", epath,
					token);
			/* oh well */
			goto ignore;
		}
		if (slotnum < 0 || slotnum >= CTX->watch_nslots) {
			...
			LIBXL__LOG(CTX, LIBXL__LOG_DEBUG, "watch epath=%s token=%s:"
					" slotnum %d out of range [0,%d>", epath, token, slotnum,
					CTX->watch_nslots);
			goto ignore;
		}
		...
		if (w->counterval != counterval) {
			LOG(DEBUG, "watch w=%p epath=%s token=%s: counter != %"PRIx32, w,
		...
		if (!xs_path_is_subpath(w->path, epath)) {
			LOG(DEBUG, "watch w=%p wpath=%s token=%s: unexpected epath=%s", w,
					w->path, token, epath);
			goto ignore;
		}

		/* At last, we have checked everything! */
		LOG(DEBUG, "watch w=%p wpath=%s token=%s: event epath=%s", w, w->path,
				token, epath);
...
int libxl__ev_xswatch_register(libxl__gc *gc, libxl__ev_xswatch *w,
		libxl__ev_xswatch_callback *func, const char *path /* copied */) {
	...
	if (LIBXL_SLIST_EMPTY(&CTX->watch_freeslots)) {
		...
		int i;
		libxl__ev_watch_slot *newarray = libxl__realloc(NOGC,
		CTX->watch_slots, sizeof(*newarray) * newarraysize);
		if (!newarray)
			goto out_nomem;
		for (i = CTX->watch_nslots; i < newarraysize; i++)
			LIBXL_SLIST_INSERT_HEAD(&CTX->watch_freeslots, &newarray[i], empty);
	...
	LOG(DEBUG, "watch w=%p wpath=%s token=%s: register slotnum=%d", w, path,
			token, slotnum);
...
void libxl__ev_xswatch_deregister(libxl__gc *gc, libxl__ev_xswatch *w) {
	...
	if (w->slotnum >= 0) {
		...
		LOG(DEBUG, "watch w=%p wpath=%s token=%s: deregister slotnum=%d", w,
				w->path, token, w->slotnum);
...
static int evtchn_revents_check(libxl__egc *egc, int revents) {
	...
	if (revents & ~POLLIN) {
		...
		LIBXL__EVENT_DISASTER(egc, "unexpected poll event on event channel fd",
				0, 0);
...
static void evtchn_fd_callback(libxl__egc *egc, libxl__ev_fd *ev, int fd,
		short events, short revents) {
	EGC_GC;
...
int libxl__ev_evtchn_wait(libxl__gc *gc, libxl__ev_evtchn *evev) {
	...
	DBG("ev_evtchn=%p port=%d wait (was waiting=%d)", evev, evev->port,
			evev->waiting);
	...
	out: evtchn_check_fd_deregister(gc);
	return rc;
}

void libxl__ev_evtchn_cancel(libxl__gc *gc, libxl__ev_evtchn *evev) {
	DBG("ev_evtchn=%p port=%d cancel (was waiting=%d)", evev, evev->port,
			evev->waiting);
...
static void devstate_callback(libxl__egc *egc, libxl__xswait_state *xsw, int rc,
		const char *sstate) {
	EGC_GC;
	...
	out: libxl__ev_devstate_cancel(gc, ds);
...
static void domaindeathcheck_abort(libxl__egc *egc, libxl__ao_abortable *abrt,
		int rc) {
...
int libxl__domaindeathcheck_start(libxl__ao *ao, libxl__domaindeathcheck *dc) {
	AO_GC;
	int rc;
	...
	rc = libxl__ev_xswatch_register(gc, &dc->watch, domaindeathcheck_callback,
			path);
	if (rc)
		goto out;
	...
	out: libxl__domaindeathcheck_stop(gc, dc);
	return rc;
}
...
static int beforepoll_internal(libxl__gc *gc, libxl__poller *poller,
		int *nfds_io, struct pollfd *fds, int *timeout_upd, struct timeval now) {
	libxl__ev_fd *efd;
	int rc;
...
#define REQUIRE_FD(req_fd_, req_events_, BODY) do{      \
	...
	if (*nfds_io) {
		...
		if (poller->fd_rindices_allocd < maxfd) {
			...
			poller->fd_rindices = libxl__realloc(NOGC, poller->fd_rindices,
					maxfd * sizeof(*poller->fd_rindices));
			memset(poller->fd_rindices + poller->fd_rindices_allocd, 0,
	...
	REQUIRE_FDS(
			{ if (used < *nfds_io) { fds[used].fd = req_fd; fds[used].events = req_events; fds[used].revents = 0; assert(req_fd < poller->fd_rindices_allocd); if (req_events & POLLIN) poller->fd_rindices[req_fd][0] = used; if (req_events & POLLPRI) poller->fd_rindices[req_fd][1] = used; if (req_events & POLLOUT) poller->fd_rindices[req_fd][2] = used; } used++; });
...
int libxl_osevent_beforepoll(libxl_ctx *ctx, int *nfds_io, struct pollfd *fds,
		int *timeout_upd, struct timeval now) {
	EGC_INIT(ctx);
	CTX_LOCK;
	int rc = beforepoll_internal(gc, ctx->poller_app, nfds_io, fds, timeout_upd,
			now);
	CTX_UNLOCK;
	EGC_FREE;
	return rc;
...
static int afterpoll_check_fd(libxl__poller *poller, const struct pollfd *fds,
		int nfds, int fd, int events)
...
static void afterpoll_internal(libxl__egc *egc, libxl__poller *poller, int nfds,
		const struct pollfd *fds, struct timeval now) {
	...
	for (;;) {
		...
		LIBXL_LIST_FOREACH(efd, &CTX->efds, entry)
		{
			...
			revents = afterpoll_check_fd(poller, fds, nfds, efd->fd,
					efd->events);
			if (revents)
				goto found_fd_event;
		}
		...
		found_fd_event: fd_occurs(egc, efd, revents);
	}

	if (afterpoll_check_fd(poller, fds, nfds, poller->wakeup_pipe[0], POLLIN)) {
		int e = libxl__self_pipe_eatall(poller->wakeup_pipe[0]);
		if (e)
			LIBXL__EVENT_DISASTER(egc, "read wakeup", e, 0);
	}
...
void libxl_osevent_register_hooks(libxl_ctx *ctx,
		const libxl_osevent_hooks *hooks, void *user) {
	GC_INIT(ctx);
	CTX_LOCK;
...
void libxl_osevent_occurred_fd(libxl_ctx *ctx, void *for_libxl, int fd,
		short events_ign, short revents_ign) {
	EGC_INIT(ctx);
	CTX_LOCK;
...
void libxl__event_disaster(libxl__egc *egc, const char *msg, int errnoval,
	...
	libxl__log(CTX, XTL_CRITICAL, errnoval, file, line, func,
			"DISASTER in event loop: %s%s%s%s", msg,
			type ? " (relates to event type " : "",
			type ? libxl_event_type_to_string(type) : "", type ? ")" : "");
...
static void egc_run_callbacks(libxl__egc *egc) {
	...
	LIBXL_TAILQ_FOREACH_SAFE(ev, &egc->occurred_for_callback, link, ev_tmp)
	{
		...
		LOG(DEBUG, "event %p callback type=%s", ev,
				libxl_event_type_to_string(ev->type));
...
libxl_event *libxl__event_new(libxl__egc *egc, libxl_event_type type,
...
static int event_check_internal(libxl__egc *egc, libxl_event **event_r,
		unsigned long typemask, libxl_event_predicate *pred, void *pred_user) {
	EGC_GC;
	libxl_event *ev;
	int rc;
...
int libxl_event_check(libxl_ctx *ctx, libxl_event **event_r, uint64_t typemask,
...
libxl__poller *libxl__poller_get(libxl__gc *gc) {
	...
	LIBXL_LIST_INSERT_HEAD(&CTX->pollers_fds_changed, p, fds_changed_entry);
	return p;
}
...
void libxl__poller_wakeup(libxl__egc *egc, libxl__poller *p) {
	...
	if (e)
		LIBXL__EVENT_DISASTER(egc, "cannot poke watch pipe", e, 0);
}
...
static int eventloop_iteration(libxl__egc *egc, libxl__poller *poller) {
	...
	for (;;) {
		...
		timeout = -1;
		rc = beforepoll_internal(gc, poller, &nfds, poller->fd_polls, &timeout,
				now);
		if (!rc)
			break;
		if (rc != ERROR_BUFFERFULL)
			goto out;

		struct pollfd *newarray =
				(nfds > INT_MAX / sizeof(struct pollfd) / 2) ?
						0 :
						libxl__realloc(NOGC, poller->fd_polls,
								sizeof(*newarray) * nfds);
...
int libxl_event_wait(libxl_ctx *ctx, libxl_event **event_r, uint64_t typemask,
		libxl_event_predicate *pred, void *pred_user) {
	int rc;
	...
	out: libxl__poller_put(ctx, poller);
...
libxl__ao *libxl__ao_create(libxl_ctx *ctx, uint32_t domid,
		const libxl_asyncop_how *how, const char *file, int line,
		const char *func) {
	libxl__ao *ao;
	...
	libxl__log(ctx, XTL_DEBUG, -1, file, line, func,
			"ao %p: create: how=%p callback=%p poller=%p", ao, how,
			ao->how.callback, ao->poller);
...
int libxl__ao_inprogress(libxl__ao *ao, const char *file, int line,
		const char *func) {
	AO_GC;
	int rc;
	...
	libxl__log(CTX, XTL_DEBUG, -1, file, line, func,
			...
			ao->constructing ? "o" : "", ao->in_initiator ? "i" : "",
			ao->complete ? "c" : "", ao->notified ? "n" : "");
...
/* Temporarily unlocks ctx, which must be locked exactly once on entry. */
{
	...
	while (!LIBXL_LIST_EMPTY(&parent->abortables)) {
		...
		LIBXL__LOG(ctx, LIBXL__LOG_DEBUG, "ao %p: abrt=%p: aborting", parent,
				abrt->ao);
	...
	out: ao__manip_leave(ctx, parent);
	return rc;
}
...
int libxl_ao_abort(libxl_ctx *ctx, const libxl_asyncop_how *how) {
	...
	found: rc = ao__abort(ctx, search);
	out: libxl__ctx_unlock(ctx);
	return rc;
}
...
int libxl__ao_abortable_register(libxl__ao_abortable *abrt) {
	...
	if (root->aborting) {
		DBG(
				"ao=%p: preemptively aborting ao_abortable registration %p (root=%p)",
				ao, abrt, root);
		return ERROR_ABORTED;
	}
...
static void dummy_asyncprogress_callback_ignore(libxl_ctx *ctx, libxl_event *ev,
		void *for_callback) {
}
...
void libxl__ao_progress_report(libxl__egc *egc, libxl__ao *ao,
	...
	if (how->callback == dummy_asyncprogress_callback_ignore) {
		...
		LOG(DEBUG, "ao %p: progress report: event queued ev=%p type=%s", ao, ev,
				libxl_event_type_to_string(ev->type));
		libxl__event_occurred(egc, ev);
	}
}
