...
static void check_open_fds(const char *what) {
	...
	for (i = 4; i < 256; i++) {
		...
		if (flags == -1) {
			if ( errno != EBADF)
				fprintf(stderr,
						"libxl: execing %s: fd %d flags returned %s (%d)\n",
		...
		if (len > 0) {
			linkpath[len] = '\0';
			fprintf(stderr,
					"libxl: execing %s: fd %d is open to %s with flags %#x\n",
...
/* call this in the child */
{
	...
	if (env != NULL) {
		for (int i = 0; env[i] != NULL && env[i + 1] != NULL; i += 2) {
			if (setenv(env[i], env[i + 1], 1) < 0) {
				LOGEV(ERROR, errno, "setting env vars (%s = %s)", env[i],
						env[i + 1]);
				goto out;
			}
		}
	...
	out: fprintf(stderr, "libxl: cannot execute %s: %s\n", arg0,
			strerror(errno));
	_exit(-1);
}

void libxl_report_child_exitstatus(libxl_ctx *ctx, xentoollog_level level,
	...
	if (WIFEXITED(status)) {
		...
		if (str)
			LIBXL__LOG(ctx, level, "%s [%ld] died due to"
					" fatal signal %s%s", what, (unsigned long )pid, str,
					coredump);
		else
			LIBXL__LOG(ctx, level, "%s [%ld] died due to unknown"
					" fatal signal number %d%s", what, (unsigned long )pid, sig,
					coredump);
	} else {
...
int libxl__spawn_record_pid(libxl__gc *gc, libxl__spawn_state *spawn, pid_t pid) {
	...
	if (r) {
		LOGE(ERROR, "write %s = %d: xenstore write failed", spawn->pidpath,
				pid);
		rc = ERROR_FAIL;
		goto out;
	}
	...
	out: return rc ? SIGTERM : 0;
}

int libxl__xenstore_child_wait_deprecated(libxl__gc *gc, uint32_t domid,
...
int libxl__spawn_spawn(libxl__egc *egc, libxl__spawn_state *ss) {
	...
	: 0;
	child = fork_replacement ? fork_replacement(CTX->childproc_user) : fork();
 ...
 * Results: Detaching or Attached Failed */
{
	...
	if (r && errno != ESRCH)
		LOGE(WARN, "%s: failed to kill intermediate child (pid=%lu)", ss->what,
				(unsigned long )child);
}
...
/* On entry, is Attached or Detaching */
{
	...
	if ((ss->rc || ss->detaching)
		...
		const char *what = GCSPRINTF(
				"%s intermediate process (startup monitor)", ss->what);
		libxl_report_child_exitstatus(CTX, loglevel, what, pid, status);
		ss->rc = ERROR_FAIL;
	} else {
		if (!status)
			LOG(ERROR, "%s [%ld]: unexpectedly exited with exit status 0,"
					" when we were waiting for it to confirm startup", ss->what,
					(unsigned long )pid);
		else if (status <= 127)
			...
			if (str)
				...
				LOG(ERROR, "%s [%ld]: died during startup due to unknown fatal"
						" signal number %d", ss->what, (unsigned long )pid,
						sig);
		}
		ss->rc = ERROR_FAIL;
	}
