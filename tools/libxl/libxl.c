...
void libxl_key_value_list_copy(libxl_ctx *ctx, libxl_key_value_list *dst,
...
int libxl__domain_rename(libxl__gc *gc, uint32_t domid, const char *old_name,
		const char *new_name, xs_transaction_t trans) {
	...
	if (!xs_write(ctx->xsh, trans, name_path, new_name, strlen(new_name))) {
		LOG(ERROR,
	...
	if (stub_dm_domid) {
		rc = libxl__domain_rename(gc, stub_dm_domid, stub_dm_old_name,
				stub_dm_new_name, trans);
		if (rc) {
	...
	x_fail: rc = ERROR_FAIL;
	goto x_rc;
	x_nomem: rc = ERROR_NOMEM;
	goto x_rc;
}

int libxl_domain_rename(libxl_ctx *ctx, uint32_t domid, const char *old_name,
		const char *new_name) {
	GC_INIT(ctx);
	int rc;
	...
	GC_FREE;
        ...
        if(!rc)
        {
            ...
            xssub_set_domain_name(domid, new_name);
            pthread_mutex_unlock(&lock_xssub);
        }

#ifdef ENABLE_XSSUB_VERBOSE
        pthread_mutex_lock(&lock_xssub);
        printf("After (attempting to) rename %s to %s...\n", old_name, new_name);
...
int libxl_domain_preserve(libxl_ctx *ctx, uint32_t domid,
		libxl_domain_create_info *info, const char *name_suffix,
		libxl_uuid new_uuid) {
	GC_INIT(ctx);
	...
	retry_transaction: t = xs_transaction_start(ctx->xsh);
...
static void xcinfo2xlinfo(libxl_ctx *ctx, const xc_domaininfo_t *xcinfo,
	...
	if (libxl_flask_sid_to_context(ctx, xlinfo->ssidref, &xlinfo->ssid_label,
			&size) < 0)
	...
	xlinfo->domain_type =
			(xcinfo->flags & XEN_DOMINF_hvm_guest) ?
...
int libxl_domain_info(libxl_ctx *ctx, libxl_dominfo *info_r, uint32_t domid) {
 ...
 */
static int cpupool_info(libxl__gc *gc, libxl_cpupoolinfo *info, uint32_t poolid,
bool exact /* exactly poolid or >= poolid */) {
	...
	rc = 0;
	out: xc_cpupool_infofree(CTX->xch, xcinfo);
	return rc;
}
...
libxl_cpupoolinfo * libxl_list_cpupool(libxl_ctx *ctx, int *nb_pool_out) {
	...
	*nb_pool_out = i;
	...
	out: libxl_cpupoolinfo_list_free(ptr, i);
	*nb_pool_out = 0;
	GC_FREE;
	return NULL;
}
...
#ifdef ENABLE_XS_SUB
            pthread_mutex_lock(&lock_xssub);
            stub = xssub_is_stubdom(ctx, info[i].domid);
            pthread_mutex_unlock(&lock_xssub);
#else
            stub = libxl_is_stubdom(ctx, info[i].domid, NULL);
#endif
...
static void remus_failover_cb(libxl__egc *egc, libxl__domain_save_state *dss,
		int rc);
...
int libxl_domain_remus_start(libxl_ctx *ctx, libxl_domain_remus_info *info,
	...
	if (libxl_defbool_val(info->colo) && libxl_defbool_val(info->compression)) {
...
static void domain_suspend_cb(libxl__egc *egc, libxl__domain_save_state *dss,
		int rc) {
	STATE_AO_GC(dss->ao);
	int flrc;
...
int libxl_domain_suspend(libxl_ctx *ctx, uint32_t domid, int fd, int flags,
	...
	rc = libxl__fd_flags_modify_save(gc, dss->fd, ~(O_NONBLOCK | O_NDELAY), 0,
			&dss->fdfl);
	if (rc < 0)
		goto out_err;
	...
	out_err: return AO_CREATE_FAIL(rc);
}
...
static void domain_death_occurred(libxl__egc *egc,
		libxl_evgen_domain_death **evg_upd, const char *why) {
	...
	for (;;) {
		...
		if (rc == -1) {
			LIBXL__EVENT_DISASTER(egc, "xc_domain_getinfolist failed while"
					" processing @releaseDomain watch event", errno, 0);
			goto out;
		}
		gotend = &domaininfos[rc];

		LOG(DEBUG, "[evg=%p:%"PRIu32"] nentries=%d rc=%d %ld..%ld", evg,
				evg->domid, nentries, rc,
		...
		for (;;) {
			...
			LOG(DEBUG,
					"[evg=%p:%"PRIu32"]" "   got=domaininfos[%d] got->domain=%ld",
			...
			if (!evg->shutdown_reported && (got->flags & XEN_DOMINF_shutdown)) {
				libxl_event *ev = NEW_EVENT(egc, DOMAIN_SHUTDOWN, got->domain,
						evg->user);

				LOG(DEBUG, " shutdown reporting");

				ev->u.domain_shutdown.shutdown_reason = (got->flags
...
void libxl__evdisable_domain_death(libxl__gc *gc, libxl_evgen_domain_death *evg) {
	CTX_LOCK;
...
void libxl_evdisable_domain_death(libxl_ctx *ctx, libxl_evgen_domain_death *evg) {
	GC_INIT(ctx);
	libxl__evdisable_domain_death(gc, evg);
	GC_FREE;
}
...
static void disk_eject_xswatch_callback(libxl__egc *egc, libxl__ev_xswatch *w,
	...
	if (libxl__xs_printf(gc, XBT_NULL, wpath, "")) {
		LIBXL__EVENT_DISASTER(egc, "xs_write failed acknowledging eject", errno,
	...
	if (rc) {
		LIBXL__EVENT_DISASTER(egc, "xs_read failed reading be_ptr_path", errno,
				LIBXL_EVENT_TYPE_DISK_EJECT);
		return;
	}
	if (!backend) {
	...
	sscanf(backend, "/local/domain/%d/backend/%" TOSTRING(BACKEND_STRING_SIZE)
	"[a-z]/%*d/%*d", &disk->backend_domid, backend_type);
...
int libxl_evenable_disk_eject(libxl_ctx *ctx, uint32_t guest_domid,
	...
	path = GCSPRINTF("%s/device/vbd/%d/eject", libxl__xs_get_dompath(gc, domid),
			devid);
	if (!path) {
		rc = ERROR_NOMEM;
		goto out;
	...
	rc = libxl__xs_read_checked(gc, XBT_NULL, GCSPRINTF("%s/dev", libxl_path),
			&configured_vdev);
	if (rc)
		goto out;
...
int libxl_domain_destroy(libxl_ctx *ctx, uint32_t domid,
		const libxl_asyncop_how *ao_how) {
	VLAD_AO_CREATE(ctx, domid, ao_how);
	...
	rc = libxl__domain_destroy(egc, dds);
	if (rc)
		return VLAD_AO_CREATE_FAIL(rc);
	else
		return VLAD_AO_INPROGRESS;
}
...
static void stubdom_destroy_callback(libxl__egc *egc,
		libxl__destroy_domid_state *dis, int rc);
...
int libxl__domain_destroy(libxl__egc *egc, libxl__domain_destroy_state *dds) {
	STATE_AO_GC(dds->ao);
	...
	return libxl__destroy_domid(egc, &dds->domain);
}
...
static void devices_destroy_cb(libxl__egc *egc,
		libxl__devices_remove_state *drs, int rc);

static void domain_destroy_domid_cb(libxl__egc *egc, libxl__ev_child *destroyer,
		pid_t pid, int status);

int libxl__destroy_domid(libxl__egc *egc, libxl__destroy_domid_state *dis) {
	STATE_AO_GC(dis->ao);
	libxl_ctx *ctx = CTX;
...
static int libxl__primary_console_find(libxl_ctx *ctx, uint32_t domid_vm,
		uint32_t *domid, int *cons_num, libxl_console_type *type) {
	GC_INIT(ctx);
...
int libxl_vncviewer_exec(libxl_ctx *ctx, uint32_t domid, int autopass) {
	...
	char *vnc_bin, *args[] = { "vncviewer",
	NULL, /* hostname:display */
	NULL, /* -autopass */
	NULL, };

	vnc_port = libxl__xs_read(gc, XBT_NULL,
			GCSPRINTF("/local/domain/%d/console/vnc-port", domid));
	if (!vnc_port) {
	...
	vnc_listen = libxl__xs_read(gc, XBT_NULL,
			GCSPRINTF("/local/domain/%d/console/vnc-listen", domid));

	if (autopass)
		vnc_pass = libxl__xs_read(gc, XBT_NULL,
				GCSPRINTF("/local/domain/%d/console/vnc-pass", domid));
...
static void device_addrm_aocomplete(libxl__egc *egc, libxl__ao_device *aodev) {
	...
	out: libxl__ao_complete(egc, ao, aodev->rc);
	return;
}
...
int libxl__device_nextid(libxl__gc *gc, uint32_t domid, char *device) {
	...
	l = libxl__xs_directory(gc, XBT_NULL,
			GCSPRINTF("%s/device/%s", libxl_dom_path, device), &nb);
	if (l == NULL || nb == 0)
		nextid = 0;
	else
...
static int libxl__device_from_vtpm(libxl__gc *gc, uint32_t domid,
		libxl_device_vtpm *vtpm, libxl__device *device) {
...
void libxl__device_vtpm_add(libxl__egc *egc, uint32_t domid,
		libxl_device_vtpm *vtpm, libxl__ao_device *aodev) {
	STATE_AO_GC(aodev->ao);
	flexarray_t *front;
	flexarray_t *back;
	...
	flexarray_append(back,
			GCSPRINTF(LIBXL_UUID_FMT, LIBXL_UUID_BYTES(vtpm->uuid)));
	...
	rc = 0;
	out: libxl__xs_transaction_abort(gc, &t);
	if (lock)
		libxl__unlock_domain_userdata(lock);
...
int libxl_device_vtpm_getinfo(libxl_ctx *ctx, uint32_t domid,
	...
	libxl_path = GCSPRINTF("%s/device/vtpm/%d", libxl__xs_libxl_path(gc, domid),
			vtpminfo->devid);
	...
	val = libxl__xs_read(gc, XBT_NULL, GCSPRINTF("%s/state", vtpmpath));
	vtpminfo->state = val ? strtoul(val, NULL, 10) : -1;

	val = libxl__xs_read(gc, XBT_NULL, GCSPRINTF("%s/event-channel", vtpmpath));
	vtpminfo->evtch = val ? strtoul(val, NULL, 10) : -1;

	val = libxl__xs_read(gc, XBT_NULL, GCSPRINTF("%s/ring-ref", vtpmpath));
	...
	val = libxl__xs_read(gc, XBT_NULL, GCSPRINTF("%s/uuid", libxl_path));
	if (val == NULL) {
...
int libxl_devid_to_device_vtpm(libxl_ctx *ctx, uint32_t domid, int devid,
		libxl_device_vtpm *vtpm) {
...
int libxl__device_from_disk(libxl__gc *gc, uint32_t domid,
		const libxl_device_disk *disk, libxl__device *device) {
	int devid;
...
static void device_disk_add(libxl__egc *egc, uint32_t domid,
		...
		char *get_vdev(libxl__gc *, void *, xs_transaction_t),
		void *get_vdev_user) {
	STATE_AO_GC(aodev->ao);
	...
	for (;;) {
		...
		case LIBXL_DISK_BACKEND_PHY:
			...
			do_backend_phy: flexarray_append(back, "params");
		...
		case LIBXL_DISK_BACKEND_QDISK:
			...
			if (libxl_defbool_val(disk->colo_enable)) {
				...
				flexarray_append(back,
						libxl__sprintf(gc, "%s", disk->colo_host));
				flexarray_append(back, "colo-port");
				flexarray_append(back,
						libxl__sprintf(gc, "%d", disk->colo_port));
				flexarray_append(back, "colo-export");
				flexarray_append(back,
						libxl__sprintf(gc, "%s", disk->colo_export));
				flexarray_append(back, "active-disk");
				flexarray_append(back,
						libxl__sprintf(gc, "%s", disk->active_disk));
				flexarray_append(back, "hidden-disk");
				flexarray_append(back,
						libxl__sprintf(gc, "%s", disk->hidden_disk));
			}
		...
		if (type == LIBXL_DOMAIN_TYPE_PV) {
			const char *protocol = xc_domain_get_native_protocol(ctx->xch,
					domid);
			if (protocol) {
...
static int libxl__device_disk_from_xenstore(libxl__gc *gc,
		const char *libxl_path, libxl_device_disk *disk) {
	libxl_ctx *ctx = libxl__gc_owner(gc);
	unsigned int len;
	char *tmp;
	int rc;
	...
	rc = libxl__xs_read_checked(gc, XBT_NULL,
			GCSPRINTF("%s/backend", libxl_path), &backend_path);
	if (rc)
		goto out;
	 ...
	 * enabled.
	 */
	tmp = xs_read(ctx->xsh, XBT_NULL, GCSPRINTF("%s/params", libxl_path), &len);
	if (tmp && strchr(tmp, ':')) {
	...
	tmp = xs_read(ctx->xsh, XBT_NULL, GCSPRINTF("%s/colo-host", libxl_path),
			&len);
	if (tmp) {
		libxl_defbool_set(&disk->colo_enable, true);
		disk->colo_host = tmp;

		tmp = xs_read(ctx->xsh, XBT_NULL, GCSPRINTF("%s/colo-port", libxl_path),
				&len);
		if (!tmp) {
...
#undef XS_READ_COLO
	...
	tmp = libxl__xs_read(gc, XBT_NULL, GCSPRINTF("%s/type", libxl_path));
	if (!tmp) {
	...
	disk->vdev = xs_read(ctx->xsh, XBT_NULL, GCSPRINTF("%s/dev", libxl_path),
			&len);
	if (!disk->vdev) {
	...
	tmp = libxl__xs_read(gc, XBT_NULL,
			libxl__sprintf(gc, "%s/removable", libxl_path));
	if (!tmp) {
	...
	tmp = libxl__xs_read(gc, XBT_NULL, GCSPRINTF("%s/device-type", libxl_path));
	if (!tmp) {
...
static int libxl__append_disk_list(libxl__gc *gc, uint32_t domid,
		libxl_device_disk **disks, int *ndisks) {
...
int libxl_device_disk_getinfo(libxl_ctx *ctx, uint32_t domid,
	...
	libxl_path = GCSPRINTF("%s/device/vbd/%d", libxl__xs_libxl_path(gc, domid),
			diskinfo->devid);
...
static char * libxl__alloc_vdev(libxl__gc *gc, void *get_vdev_user,
	...
	do {
		...
		if (libxl__xs_read(gc, t,
				GCSPRINTF("%s/device/vbd/%d/backend", libxl_dom_path,
						devid)) == NULL) {
			if (errno == ENOENT)
...
char *libxl__device_disk_find_local_path(libxl__gc *gc, libxl_domid guest_domid,
	 ...
	 */
	if (disk->format == LIBXL_DISK_FORMAT_RAW && disk->script == NULL) {
...
int libxl__device_nic_setdefault(libxl__gc *gc, libxl_device_nic *nic,
	...
	if (!nic->script
			&& asprintf(&nic->script, "%s/vif-bridge",
...
static int libxl__device_from_nic(libxl__gc *gc, uint32_t domid,
		libxl_device_nic *nic, libxl__device *device) {
...
void libxl__device_nic_add(libxl__egc *egc, uint32_t domid,
	...
	if (nic->script)
		flexarray_append_pair(back, "script",
				libxl__abs_path(gc, nic->script, libxl__xen_script_dir_path()));
	...
	if (nic->rate_interval_usecs > 0) {
		...
		flexarray_append(back,
				GCSPRINTF("%"PRIu64",%"PRIu32"", nic->rate_bytes_per_interval,
	...
	flexarray_append(back, "type");
	flexarray_append(back,
			libxl__strdup(gc, libxl_nic_type_to_string(nic->nictype)));
	...
	flexarray_append(front,
			GCSPRINTF(LIBXL_MAC_FMT, LIBXL_MAC_BYTES(nic->mac)));
...
static int libxl__device_nic_from_xenstore(libxl__gc *gc,
		const char *libxl_path, libxl_device_nic *nic) {
	const char *tmp;
	int rc;
...
static int libxl__append_nic_list(libxl__gc *gc, uint32_t domid,
		libxl_device_nic **nics, int *nnics) {
	char *libxl_dir_path = NULL;
	char **dir = NULL;
	unsigned int n = 0;
...
int libxl_device_nic_getinfo(libxl_ctx *ctx, uint32_t domid,
	...
	libxl_path = GCSPRINTF("%s/device/vif/%d", libxl__xs_libxl_path(gc, domid),
			nicinfo->devid);
...
const char *libxl__device_nic_devname(libxl__gc *gc, uint32_t domid,
		uint32_t devid, libxl_nic_type type) {
	switch (type) {
...
int libxl__device_console_add(libxl__gc *gc, uint32_t domid,
		libxl__device_console *console, libxl__domain_build_state *state,
		libxl__device *device) {
...
int libxl__init_console_from_channel(libxl__gc *gc,
		libxl__device_console *console, int dev_num,
...
static int libxl__device_channel_from_xenstore(libxl__gc *gc,
		const char *libxl_path, libxl_device_channel *channel) {
	const char *tmp;
	int rc;
...
static int libxl__append_channel_list(libxl__gc *gc, uint32_t domid,
		libxl_device_channel **channels, int *nchannels) {
	char *libxl_dir_path = NULL;
	char **dir = NULL;
...
libxl_device_channel *libxl_device_channel_list(libxl_ctx *ctx, uint32_t domid,
		int *num) {
	GC_INIT(ctx);
...
int libxl_device_channel_getinfo(libxl_ctx *ctx, uint32_t domid,
		libxl_device_channel *channel, libxl_channelinfo *channelinfo) {
	GC_INIT(ctx);
	char *dompath, *fe_path, *libxl_path;
	char *val;
	int rc;
	...
	libxl_path = GCSPRINTF("%s/device/console/%d",
			libxl__xs_libxl_path(gc, domid), channelinfo->devid + 1);
...
static int libxl__device_from_vkb(libxl__gc *gc, uint32_t domid,
		libxl_device_vkb *vkb, libxl__device *device) {
...
int libxl_device_vkb_add(libxl_ctx *ctx, uint32_t domid, libxl_device_vkb *vkb,
	...
	out: libxl__ao_complete(egc, ao, rc);
	return AO_INPROGRESS;
}

int libxl__device_vkb_add(libxl__gc *gc, uint32_t domid, libxl_device_vkb *vkb) {
	flexarray_t *front;
	flexarray_t *back;
	libxl__device device;
	int rc;
...
static int libxl__device_from_vfb(libxl__gc *gc, uint32_t domid,
		libxl_device_vfb *vfb, libxl__device *device) {
...
typedef struct libxl__ddomain_device {
	libxl__device *dev;
	LIBXL_SLIST_ENTRY(struct libxl__ddomain_device)
	next;
...
typedef struct libxl__ddomain_guest {
	...
	LIBXL_SLIST_HEAD(, struct libxl__ddomain_device)
	devices;
	LIBXL_SLIST_ENTRY(struct libxl__ddomain_guest)
	next;
...
typedef struct {
	...
	LIBXL_SLIST_HEAD(, struct libxl__ddomain_guest)
	guests;
} libxl__ddomain;
...
static void device_complete(libxl__egc *egc, libxl__ao_device *aodev) {
	...
	LOG(DEBUG, "device %s %s %s", libxl__device_backend_path(gc, aodev->dev),
			libxl__device_action_to_string(aodev->action),
...
static int add_device(libxl__egc *egc, libxl__ao *ao,
		libxl__ddomain_guest *dguest, libxl__ddomain_device *ddev) {
	AO_GC;
...
static void backend_watch_callback(libxl__egc *egc, libxl__ev_xswatch *watch,
	...
	*p = '\0';
	...
	if (ddev == NULL && state == XenbusStateClosed) {
		 ...
		 */
		LIBXL_SLIST_REMOVE(&dguest->devices, ddev, libxl__ddomain_device, next);
	...
	skip: libxl__nested_ao_free(nested_ao);
	free(dev);
	free(ddev);
	free(dguest);
	return;
...
int libxl_device_events_handler(libxl_ctx *ctx, const libxl_asyncop_how *ao_how) {
...
static int libxl__fill_dom0_memory_info(libxl__gc *gc, uint32_t *target_memkb,
	...
	if (staticmax == NULL) {
		libxl__xs_printf(gc, t, max_path, "%"PRIu32, (uint32_t) info.max_memkb);
		*max_memkb = (uint32_t) info.max_memkb;
	}

	rc = 0;

	out: if (!xs_transaction_end(ctx->xsh, t, 0)) {
		if (errno == EAGAIN)
			goto retry_transaction;
		else
			rc = ERROR_FAIL;
...
int libxl_set_memory_target(libxl_ctx *ctx, uint32_t domid,
	...
	videoram_s = libxl__xs_read(gc, t,
			GCSPRINTF("%s/memory/videoram", dompath));
	...
	if (new_target_memkb > memorykb) {
		LOG(ERROR, "memory_dynamic_max must be less than or equal to"
				" memory_static_max\n");
		abort_transaction = 1;
		goto out;
	}
	...
	printf("new_target_memkb: %d\n", new_target_memkb);
	rc = xc_domain_set_pod_target(ctx->xch, domid,
			(new_target_memkb + LIBXL_MAXMEM_CONSTANT) / 4, NULL, NULL, NULL);
	if (rc != 0) {
		LOGE(ERROR,
				"xc_domain_set_pod_target domid=%d, memkb=%d " "failed rc=%d\n",
				domid, new_target_memkb / 4, rc);
		abort_transaction = 1;
		goto out;
	}

	libxl__xs_printf(gc, t, GCSPRINTF("%s/memory/target", dompath), "%"PRIu32,
			new_target_memkb);

	libxl_dominfo_init(&ptr);
	xcinfo2xlinfo(ctx, &info, &ptr);
	uuid = libxl__uuid2string(gc, ptr.uuid);
	libxl__xs_printf(gc, t, GCSPRINTF("/vm/%s/memory", uuid), "%"PRIu32,
			new_target_memkb / 1024);
	libxl_dominfo_dispose(&ptr);

	out: if (!xs_transaction_end(ctx->xsh, t, abort_transaction)
			&& !abort_transaction)
		if (errno == EAGAIN)
			goto retry_transaction;

	out_no_transaction: if (lock)
		libxl__unlock_domain_userdata(lock);
	CTX_UNLOCK;
	GC_FREE;
	return rc;
...
static int libxl__get_memory_target(libxl__gc *gc, uint32_t domid,
		uint32_t *out_target_memkb, uint32_t *out_max_memkb) {
	int rc;
	char *target = NULL, *static_max = NULL, *endptr = NULL;
	char *dompath = libxl__xs_get_dompath(gc, domid);
	uint32_t target_memkb, max_memkb;

	target = libxl__xs_read(gc, XBT_NULL,
			GCSPRINTF("%s/memory/target", dompath));
	...
	if ((!target || !static_max) && !domid) {
		...
		LOGE(ERROR, "cannot get target memory info from %s/memory/static-max",
				dompath);
		goto out;
	} else {
		target_memkb = strtoul(target, &endptr, 10);
		...
		if (*endptr != '\0') {
			LOGE(ERROR, "invalid memory target %s from %s/memory/static-max\n",
					static_max, dompath);
			goto out;
		}
...
int libxl_wait_for_free_memory(libxl_ctx *ctx, uint32_t domid,
		uint32_t memory_kb, int wait_secs) {
	int rc = 0;
	libxl_physinfo info;
	GC_INIT(ctx);
...
bool increase) {
	...
	libxl_dominfo_init(&info);
	prev_memkb = increase ? UINT64_MAX : 0;
	...
	do {
		...
		if (increase ?
				current_memkb > prev_memkb : current_memkb < prev_memkb) {
			rc = ERROR_FAIL;
			goto out;
		...
		prev_memkb = current_memkb;
			...
			&& (increase ?
					current_memkb > target_memkb : current_memkb < target_memkb));
	...
	out: libxl_dominfo_dispose(&info);
	return rc;
}
...
int libxl_get_physinfo(libxl_ctx *ctx, libxl_physinfo *physinfo) {
	...
	physinfo->cap_hvm_directio = !!(xcphysinfo.capabilities
			& XEN_SYSCTL_PHYSCAP_hvm_directio);
...
libxl_vcpuinfo *libxl_list_vcpu(libxl_ctx *ctx, uint32_t domid,
	...
	*nr_cpus_out = libxl_get_max_cpus(ctx);
	...
	for (*nr_vcpus_out = 0; *nr_vcpus_out <= domaininfo.max_vcpu_id;
			++*nr_vcpus_out, ++ptr) {
		...
		if (xc_vcpu_getaffinity(ctx->xch, domid, *nr_vcpus_out, ptr->cpumap.map,
				ptr->cpumap_soft.map,
	...
	err: libxl_bitmap_dispose(&ptr->cpumap);
...
static int libxl__set_vcpuaffinity(libxl_ctx *ctx, uint32_t domid,
		uint32_t vcpuid, const libxl_bitmap *cpumap_hard,
		const libxl_bitmap *cpumap_soft, unsigned flags) {
	GC_INIT(ctx);
	...
	if (xc_vcpu_setaffinity(ctx->xch, domid, vcpuid,
			cpumap_hard ? hard.map : NULL, cpumap_soft ? soft.map : NULL,
			flags)) {
	 ...
	 */
	if (cpumap_hard && !libxl_bitmap_equal(cpumap_hard, &hard, 0))
...
int libxl_set_vcpuaffinity(libxl_ctx *ctx, uint32_t domid, uint32_t vcpuid,
		const libxl_bitmap *cpumap_hard, const libxl_bitmap *cpumap_soft) {
	return libxl__set_vcpuaffinity(ctx, domid, vcpuid, cpumap_hard, cpumap_soft,
			0);
}
...
int libxl_set_vcpuaffinity_force(libxl_ctx *ctx, uint32_t domid,
	...
	return libxl__set_vcpuaffinity(ctx, domid, vcpuid, cpumap_hard, cpumap_soft,
	XEN_VCPUAFFINITY_FORCE);
}

int libxl_set_vcpuaffinity_all(libxl_ctx *ctx, uint32_t domid,
		unsigned int max_vcpus, const libxl_bitmap *cpumap_hard,
		const libxl_bitmap *cpumap_soft) {
	GC_INIT(ctx);
	int i, rc = 0;
...
static int libxl__set_vcpuonline_xenstore(libxl__gc *gc, uint32_t domid,
		libxl_bitmap *cpumap, const libxl_dominfo *info) {
	char *dompath;
	xs_transaction_t t;
	int i, rc = ERROR_FAIL;
	...
	for (i = 0; i <= info->vcpu_max_id; i++)
		libxl__xs_printf(gc, t, GCSPRINTF("%s/cpu/%u/availability", dompath, i),
...
int libxl_set_vcpuonline(libxl_ctx *ctx, uint32_t domid, libxl_bitmap *cpumap) {
	...
	if (maxcpus > info.vcpu_max_id + 1) {
		LOGE(ERROR, "Requested %d VCPUs, however maxcpus is %d!", maxcpus,
				info.vcpu_max_id + 1);
		rc = ERROR_FAIL;
		goto out;
	}
...
static int sched_rtds_vcpu_set(libxl__gc *gc, uint32_t domid,
	...
	r = xc_sched_rtds_vcpu_set(CTX->xch, domid, vcpus, scinfo->num_vcpus);
	if (r != 0) {
...
static int sched_rtds_vcpu_set_all(libxl__gc *gc, uint32_t domid,
	...
	r = xc_sched_rtds_vcpu_set(CTX->xch, domid, vcpus, num_vcpus);
	if (r != 0) {
		LOGE(ERROR, "setting vcpu sched rtds");
		rc = ERROR_FAIL;
		goto out;
...
int libxl_vcpu_sched_params_set(libxl_ctx *ctx, uint32_t domid,
	...
	case LIBXL_SCHEDULER_ARINC653:
		LOG(ERROR,
				"per-VCPU parameter setting not supported for this scheduler");
		rc = ERROR_INVAL;
		break;
	case LIBXL_SCHEDULER_RTDS:
...
int libxl_vcpu_sched_params_get(libxl_ctx *ctx, uint32_t domid,
	...
	case LIBXL_SCHEDULER_ARINC653:
		LOG(ERROR,
				"per-VCPU parameter getting not supported for this scheduler");
		rc = ERROR_INVAL;
		break;
	case LIBXL_SCHEDULER_RTDS:
...
int libxl_send_trigger(libxl_ctx *ctx, uint32_t domid, libxl_trigger trigger,
		uint32_t vcpuid) {
	int rc;
	GC_INIT(ctx);
...
int libxl_send_sysrq(libxl_ctx *ctx, uint32_t domid, char sysrq) {
	...
	libxl__xs_printf(gc, XBT_NULL, GCSPRINTF("%s/control/sysrq", dompath), "%c",
			sysrq);
 ...
 * if it wishes. */
int libxl_xen_console_read_line(libxl_ctx *ctx, libxl_xen_console_reader *cr,
		char **line_r) {
	int ret;
	GC_INIT(ctx);

	memset(cr->buffer, 0, cr->size);
	ret = xc_readconsolering(ctx->xch, cr->buffer, &cr->count, cr->clear,
			cr->incremental, &cr->index);
	if (ret < 0) {
...
void libxl_xen_console_read_finish(libxl_ctx *ctx, libxl_xen_console_reader *cr) {
	free(cr->buffer);
	free(cr);
}
...
uint32_t libxl_vm_get_start_time(libxl_ctx *ctx, uint32_t domid) {
	...
	vm_path = libxl__xs_read(gc, XBT_NULL, GCSPRINTF("%s/vm", dompath));
	start_time = libxl__xs_read(gc, XBT_NULL,
			GCSPRINTF("%s/start_time", vm_path));
	if (start_time == NULL) {
...
int libxl_tmem_shared_auth(libxl_ctx *ctx, uint32_t domid, char* uuid, int auth) {
	int rc;
	GC_INIT(ctx);
...
int libxl_tmem_freeable(libxl_ctx *ctx) {
	...
	rc = xc_tmem_control(ctx->xch, -1, XEN_SYSCTL_TMEM_OP_QUERY_FREEABLE_MB, -1,
			0, 0, 0);
	if (rc < 0) {
...
int libxl_cpupool_create(libxl_ctx *ctx, const char *name,
		libxl_scheduler sched, libxl_bitmap cpumap, libxl_uuid *uuid,
		uint32_t *poolid) {
	GC_INIT(ctx);
	int rc;
	int i;
	...
	for (;;) {
		...
		libxl__xs_printf(gc, t, GCSPRINTF("/local/pool/%d/uuid", *poolid), "%s",
				uuid_string);
		libxl__xs_printf(gc, t, GCSPRINTF("/local/pool/%d/name", *poolid), "%s",
				name);
...
int libxl_cpupool_destroy(libxl_ctx *ctx, uint32_t poolid) {
	...
	out1: libxl_bitmap_dispose(&cpumap);
	out: xc_cpupool_infofree(ctx->xch, info);
	GC_FREE;
...
int libxl_cpupool_cpuremove_node(libxl_ctx *ctx, uint32_t poolid, int node,
	...
	*cpus = 0;
	...
	out: libxl_cpupoolinfo_list_free(poolinfo, n_pools);
...
static int fd_set_flags(libxl_ctx *ctx, int fd, int fcntlgetop, int fcntlsetop,
		const char *fl, int flagmask, int set_p) {
	int flags, r;
	GC_INIT(ctx);
...
int libxl__fd_flags_modify_save(libxl__gc *gc, int fd, int mask, int val,
		int *r_oldflags) {
	int rc, ret, fdfl;
...
#define MERGE(type, ptr, compare, merge)                                \
	...
	MERGE(disk, disks, COMPARE_DISK,
			{ if (src->removable) { if (!src->pdev_path || *src->pdev_path == '\0') {
			/* 1, no media in drive */
			free(dst->pdev_path); dst->pdev_path = libxl__strdup(NOGC, ""); dst->format = LIBXL_DISK_FORMAT_EMPTY; } else {
