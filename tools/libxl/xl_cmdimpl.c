...
__thread xlchild children[child_max];

#define INVALID_DOMID ~0

static volatile int nb_active_threads = 0;

static __thread const char *common_domname;
static int fd_lock = -1;

static uint32_t free_memkb;
static pthread_mutex_t free_mem_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static const char migrate_receiver_ready[] = "domain received, ready to unpause";
static const char migrate_permission_to_go[] =
		"domain is yours, you are cleared to unpause";
static const char migrate_report[] = "my copy unpause results are as follows";
...
static inline int acquire_lock(void) {
	int rc;
	struct flock fl;
	...
	get_lock: rc = fcntl(fd_lock, F_SETLKW, &fl);
	if (rc < 0 && errno == EINTR)
		goto get_lock;
	if (rc < 0) {
...
static inline int release_lock(void) {
	int rc;
	struct flock fl;
	...
	release_lock: fl.l_type = F_UNLCK;
	fl.l_whence = SEEK_SET;
	fl.l_start = 0;
	fl.l_len = 0;
...
static void dolog(const char *file, int line, const char *func, char *fmt, ...) __attribute__((format(printf,4,5)));
...
static void xvasprintf(char **strp, const char *fmt, va_list ap) __attribute__((format(printf,2,0)));
...
static void xasprintf(char **strp, const char *fmt, ...) __attribute__((format(printf,2,3)));
...
static void flush_stream(FILE *fh) {
	const char *fh_name =
			fh == stdout ? "stdout" :
			fh == stderr ? "stderr" : (abort(), (const char*) 0);
...
static void printf_info(enum output_format output_format, int domid,
	...
	out: yajl_gen_free(hand);

	if (s != yajl_gen_status_ok)
		fprintf(stderr, "unable to format domain config as JSON (YAJL:%d)\n",
				s);
...
static int parse_action_on_shutdown(const char *buf,
	...
	for (i = 0;
			i
					< sizeof(action_on_shutdown_names)
...
static void parse_disk_config_multistring(XLU_Config **config, int nspecs,
		const char * const *specs, libxl_device_disk *disk) {
	int e;
...
static void split_string_into_string_list(const char *str, const char *delim,
...
static void trim(char_predicate_t predicate, const char *input, char **output) {
	...
	for (first = input; *first && predicate((unsigned char) first[0]); first++)
		;

	for (after = first + strlen(first);
			after > first && predicate((unsigned char) after[-1]); after--)
		;
...
static int split_string_into_pair(const char *str, const char *delim, char **a,
		char **b) {
...
static int update_cpumap_range(const char *str, libxl_bitmap *cpumap) {
	...
	while (ida <= idb) {
		if (is_nodes) {
			...
			/* Add/Remove this cpu */
			is_not ?
					libxl_bitmap_reset(cpumap, ida) :
					libxl_bitmap_set(cpumap, ida);
		}
		ida++;
	}

	out: libxl_bitmap_dispose(&node_cpumap);
	return rc;
}
...
static void parse_top_level_vnc_options(XLU_Config *config, libxl_vnc_info *vnc) {
	long l;
...
static void parse_top_level_sdl_options(XLU_Config *config, libxl_sdl_info *sdl) {
...
static void parse_vcpu_affinity(libxl_domain_build_info *b_info,
		XLU_ConfigList *cpus, const char *buf, int num_cpus, bool is_hard) {
...
static int match_option_size(const char *prefix, size_t len, char *arg,
		char **argopt) {
...
static void parse_vnuma_config(const XLU_Config *config,
	...
	for (i = 0; i < b_info->num_vnuma_nodes; i++) {
		...
		p->distances = xcalloc(b_info->num_vnuma_nodes, sizeof(*p->distances));
	...
	for (i = 0; i < num_vnuma; i++) {
		...
		for (conf_count = 0;
				(conf_option = xlu_cfg_get_listitem2(vnode_config_list,
			...
			if (xlu_cfg_value_type(conf_option) == XLU_STRING) {
				...
				if (split_string_into_pair(buf, "=", &option_untrimmed,
						&value_untrimmed)) {
				...
				if (!strcmp("pnode", option)) {
					...
					if (val >= nr_nodes) {
						fprintf(stderr, "xl: invalid pnode number: %lu\n", val);
						exit(EXIT_FAILURE);
					}
					p->pnode = val;
					libxl_defbool_set(&b_info->numa_placement, false);
...
static void parse_config_data(const char *config_source,
		const char *config_data, int config_len, libxl_domain_config *d_config) {
	const char *buf;
	long l, vcpus = 0;
	XLU_Config *config;
	...
	if (!xlu_cfg_get_string(config, "init_seclabel", &buf, 0))
		xlu_cfg_replace_string(config, "init_seclabel", &c_info->ssid_label, 0);

	if (!xlu_cfg_get_string(config, "seclabel", &buf, 0)) {
		if (c_info->ssid_label)
			xlu_cfg_replace_string(config, "seclabel", &b_info->exec_ssid_label,
					0);
		else
			xlu_cfg_replace_string(config, "seclabel", &c_info->ssid_label, 0);
	}
	...
	b_info->shadow_memkb =
			!xlu_cfg_get_long(config, "shadow_memory", &l, 0) ?
					l * 1024 :
	...
	if (!xlu_cfg_get_long(config, "tsc_mode", &l, 1)) {
		...
		fprintf(stderr,
				...
				s ? "e.g. tsc_mode=\"" : "", s ? s : "", s ? "\"" : "");

		if (l < LIBXL_TSC_MODE_DEFAULT || l > LIBXL_TSC_MODE_NATIVE_PARAVIRT) {
	...
	case LIBXL_DOMAIN_TYPE_HVM:
		...
		switch (xlu_cfg_get_list(config, "viridian", &viridian, &num_viridian,
				1)) {
		case 0: /* Success */
			if (num_viridian) {
		...
		if (!xlu_cfg_get_long(config, "mmio_hole", &l, 0)) {
			...
			if (mmio_hole_size < HVM_BELOW_4G_MMIO_LENGTH
				...
				fprintf(stderr, "ERROR: invalid value %ld for \"mmio_hole\"\n",
						l);
				exit(1);
			}
		}
	...
	if (!xlu_cfg_get_list(config, "irqs", &irqs, &num_irqs, 0)) {
		...
		for (i = 0; i < num_irqs; i++) {
			...
			if (!buf) {
				fprintf(stderr, "xl: Unable to get element %d in irq list\n",
						i);
				exit(1);
			}
			ul = strtoul(buf, &ep, 10);
			if (ep == buf || *ep != '\0') {
				fprintf(stderr, "xl: Invalid argument parsing irq: %s\n", buf);
				exit(1);
			}
			if (ul >= UINT32_MAX) {
	...
	if (!xlu_cfg_get_list(config, "iomem", &iomem, &num_iomem, 0)) {
		...
		for (i = 0; i < num_iomem; i++) {
			...
			if (!buf) {
				fprintf(stderr, "xl: Unable to get element %d in iomem list\n",
						i);
				exit(1);
			}
			libxl_iomem_range_init(&b_info->iomem[i]);
			ret = sscanf(buf, "%" SCNx64",%" SCNx64"%n@%" SCNx64"%n",
					&b_info->iomem[i].start, &b_info->iomem[i].number, &used,
					&b_info->iomem[i].gfn, &used);
			if (ret < 2 || buf[used] != '\0') {
				fprintf(stderr, "xl: Invalid argument parsing iomem: %s\n",
						buf);
				exit(1);
			}
		}
	...
	if (!xlu_cfg_get_list(config, "disk", &vbds, 0, 0)) {
		...
		while ((buf = xlu_cfg_get_listitem(vbds, d_config->num_disks)) != NULL) {
			...
			disk = ARRAY_EXTEND_INIT_NODEVID(d_config->disks,
					d_config->num_disks, libxl_device_disk_init);
	...
	if (!xlu_cfg_get_list(config, "vtpm", &vtpms, 0, 0)) {
		...
		while ((buf = xlu_cfg_get_listitem(vtpms, d_config->num_vtpms)) != NULL) {
			...
			vtpm = ARRAY_EXTEND_INIT(d_config->vtpms, d_config->num_vtpms,
	...
	if (!xlu_cfg_get_list(config, "channel", &channels, 0, 0)) {
		...
		while ((buf = xlu_cfg_get_listitem(channels, d_config->num_channels))
			...
			for (i = 0; i < len; i++) {
				...
				int rc;
				rc = split_string_into_pair(pairs[i], "=", &key_untrimmed,
						&value_untrimmed);
				if (rc != 0) {
	...
	if (!xlu_cfg_get_list(config, "vif", &nics, 0, 0)) {
		...
		while ((buf = xlu_cfg_get_listitem(nics, d_config->num_nics)) != NULL) {
			...
			nic = ARRAY_EXTEND_INIT(d_config->nics, d_config->num_nics,
	...
	if (!xlu_cfg_get_list(config, "vif2", NULL, 0, 0)) {
		fprintf(stderr,
				"WARNING: vif2: netchannel2 is deprecated and not supported by xl\n");
	}
	...
	if (!xlu_cfg_get_list(config, "pci", &pcis, 0, 0)) {
		...
		for (i = 0; (buf = xlu_cfg_get_listitem(pcis, i)) != NULL; i++) {
			...
			pcidev = ARRAY_EXTEND_INIT_NODEVID(d_config->pcidevs,
					d_config->num_pcidevs, libxl_device_pci_init);
	...
	if (!xlu_cfg_get_list(config, "dtdev", &dtdevs, 0, 0)) {
		...
		for (i = 0; (buf = xlu_cfg_get_listitem(dtdevs, i)) != NULL; i++) {
			...
			dtdev = ARRAY_EXTEND_INIT_NODEVID(d_config->dtdevs,
					d_config->num_dtdevs, libxl_device_dtdev_init);
	...
	if (!xlu_cfg_get_list(config, "usbctrl", &usbctrls, 0, 0)) {
		...
		while ((buf = xlu_cfg_get_listitem(usbctrls, d_config->num_usbctrls))
			...
			usbctrl = ARRAY_EXTEND_INIT(d_config->usbctrls,
					d_config->num_usbctrls, libxl_device_usbctrl_init);
			p = strtok(buf2, ",");
			if (!p)
				goto skip_usbctrl;
			do {
	...
	if (!xlu_cfg_get_list(config, "usbdev", &usbdevs, 0, 0)) {
		...
		while ((buf = xlu_cfg_get_listitem(usbdevs, d_config->num_usbdevs))
			...
			usbdev = ARRAY_EXTEND_INIT_NODEVID(d_config->usbdevs,
					d_config->num_usbdevs, libxl_device_usbdev_init);
			p = strtok(buf2, ",");
			if (!p)
				goto skip_usbdev;
			do {
	...
	if (!xlu_cfg_get_string(config, "device_model_version", &buf, 0)) {
		if (!strcmp(buf, "qemu-xen-traditional")) {
			...
			fprintf(stderr, "Unknown device_model_version \"%s\" specified\n",
					buf);
			exit(1);
		}
	...
	if (!xlu_cfg_get_string(config, "device_model_stubdomain_seclabel", &buf,
			0))
...
#undef parse_extra_args
	...
	if (c_info->type == LIBXL_DOMAIN_TYPE_HVM) {
		if (!xlu_cfg_get_string(config, "vga", &buf, 0)) {
			...
			b_info->u.hvm.vga.kind =
					l ? LIBXL_VGA_INTERFACE_TYPE_STD : LIBXL_VGA_INTERFACE_TYPE_CIRRUS;
		...
		xlu_cfg_replace_string(config, "spicehost", &b_info->u.hvm.spice.host,
				0);
		...
		case EINVAL:
			...
			if (!xlu_cfg_replace_string(config, "serial", &b_info->u.hvm.serial,
					0))
				break;
			/* FALLTHRU */
		default:
		...
		if (b_info->u.hvm.vnc.listen && b_info->u.hvm.vnc.display
				&& strchr(b_info->u.hvm.vnc.listen, ':') != NULL) {
			fprintf(stderr, "ERROR: Display specified both in vnclisten"
					" and vncdisplay!\n");
			exit(1);
		...
		if (!xlu_cfg_get_string(config, "vendor_device", &buf, 0)) {
			...
			if (e) {
				fprintf(stderr, "xl: unknown vendor_device '%s'\n", buf);
				exit(-ERROR_FAIL);
			}
	...
	if (!xlu_cfg_get_string(config, "gic_version", &buf, 1)) {
		...
		if (e) {
			fprintf(stderr, "Unknown gic_version \"%s\" specified\n", buf);
			exit(-ERROR_FAIL);
		}
	}
...
static void reload_domain_config(uint32_t domid, libxl_domain_config *d_config) {
	int rc;
	uint8_t *t_data;
	int ret, t_len;
	...
	if (t_len > 0) {
		...
		parse_config_data("<updated>", (const char *) t_data, t_len, d_config);
		free(t_data);
	...
	if (rc) {
		LOG(
				"failed to retrieve guest configuration (rc=%d). " "reusing old configuration",
				rc);
...
static domain_restart_type handle_domain_death(uint32_t *r_domid,
		libxl_event *event, libxl_domain_config *d_config) {
...
static int preserve_domain(uint32_t *r_domid, libxl_event *event,
	...
	LOG("Preserving domain %d %s with suffix%s", *r_domid,
			d_config->c_info.name, strtime);
	rc = libxl_domain_preserve(ctx, *r_domid, &d_config->c_info, strtime,
			new_uuid);
...
static bool freemem(uint32_t domid, libxl_domain_build_info *b_info) {
	...
	log_time("Enter libxl_domain_need_memory");
	//need_memkb == the "memory" written in config_file
	rc = libxl_domain_need_memory(ctx, b_info, &need_memkb);
	log_time("Exit libxl_domain_need_memory");
	if (rc < 0)
		return false;

	//do {
	pthread_mutex_lock(&free_mem_mutex);
	if (free_memkb >= need_memkb) {
		free_memkb -= need_memkb;
		pthread_mutex_unlock(&free_mem_mutex);
		return true;
	} else {
		req_memkb = free_memkb - need_memkb;
		free_memkb = 0;
		pthread_mutex_unlock(&free_mem_mutex);
	}

	log_time("Enter libxl_set_memory_target");
	rc = libxl_set_memory_target(ctx, 0, req_memkb, 1, 0); 
	log_time("Exit libxl_set_memory_target");
	if (rc < 0)
		return false;

	/* wait until dom0 reaches its target, as long as we are making
	 * progress */
	log_time("Enter libxl_wait_for_memory_target");
	rc = libxl_wait_for_memory_target(ctx, 0, 10, true);
	log_time("Exit libxl_wait_for_memory_target");
	if (rc < 0)
		return false;
...
static void autoconnect_console(libxl_ctx *ctx_ignored, libxl_event *ev,
		void *priv) {
...
static int domain_wait_event(uint32_t domid, libxl_event **event_r) {
	...
	for (;;) {
		...
		if ((*event_r)->domid != domid) {
			...
			LOG(
					"INTERNAL PROBLEM - ignoring unexpected event for" " domain %d (expected %d): event=%s",
...
static int create_domain(struct domain_create *dom_info) {
	...
	if (restoring) {
		...
		CHK_ERRNOVAL(
				libxl_read_exactly(ctx, restore_fd, &hdr, sizeof(hdr),
		...
		fprintf(stderr, "Loading new save file %s"
				...
				hdr.mandatory_flags, hdr.optional_flags, hdr.optional_data_len);
		...
		if (hdr.optional_data_len) {
			...
			CHK_ERRNOVAL(
					libxl_read_exactly(ctx, restore_fd, optdata_begin,
...
#define WITH_OPTDATA(amt, body)                                 \
	...
	if (config_file) {
		...
		if (!strcmp(config_file, "/dev/null")) {
			...
			ret = libxl_read_file_contents(ctx, config_file, &config_data,
					&config_len);
			if (ret) {
		...
		if (!restoring && extra_config && strlen(extra_config)) {
			...
			config_data = xrealloc(config_data,
					config_len + strlen(extra_config) + 2 + 1);
	...
	//log_time("Enter acquire_lock");
	//rc = acquire_lock(); 
	//log_time("Exit acquire_lock");
	//if (rc < 0)
	//	goto error_out;

	__sync_add_and_fetch(&nb_active_threads, 1);
	/*block executed only by one thread*/
	...
	if (!flocked) {
		...
		log_time("Enter libxl_get_free_memory");

		//sum avail[node][zone] pages for all mem zones
		pthread_mutex_lock(&free_mem_mutex);
		rc = libxl_get_free_memory(ctx, &free_memkb);
		pthread_mutex_unlock(&free_mem_mutex);

		log_time("Exit libxl_get_free_memory");
		if (rc < 0) {
			pthread_mutex_unlock(&flock);
			goto error_out;
		}
	...
	if (restoring) {
		...
		ret = libxl_domain_create_restore(ctx, &d_config, &domid, restore_fd,
				send_back_fd, &params, 0, autoconnect_console_how);
		...
		/* Do soft reset. */
		ret = libxl_domain_soft_reset(ctx, &d_config, domid_soft_reset, 0,
				autoconnect_console_how);
		domid = domid_soft_reset;
		domid_soft_reset = INVALID_DOMID;
	} else {
		log_time("Enter libxl_domain_create_new");
		ret = libxl_domain_create_new(ctx, &d_config, &domid, 0,
				autoconnect_console_how);
		log_time("Exit libxl_domain_create_new");
	}
	if (ret)
		goto error_out;

	/*VLAD
	 * the lock should be released by the last thread passing this code*/
	if (!__sync_sub_and_fetch(&nb_active_threads, 1)) {
#ifdef DOM0_RECLAIM
		...
		flocked = false;
		pthread_mutex_unlock(&flock);
		log_time("Enter release_lock");
		release_lock();
		log_time("Exit release_lock");
	}
	...
	if (!paused) {
		log_time("Enter libxl_domain_unpause");
		libxl_domain_unpause(ctx, domid);
		log_time("Exit libxl_domain_unpause");
	}
	...
	//need_daemon is true by default
	if (need_daemon) {
		char *name;
	...
	while (1) {
		...
		default:
			...
			LOG("warning, got unexpected event type %d, event=%s", event->type,
					evstr);
			free(evstr);
		}
...
#if defined(ENABLE_XS_SUB) && defined(ENABLE_XSSUB_VERBOSE)
        pthread_mutex_lock(&lock_xssub);
        printf("After (attempting to) create a new domain...\n");
        xssub_print_all_domains();
        pthread_mutex_unlock(&lock_xssub);
#endif        
	return ret;
}
	...
	if (!command || !strcmp(command, "help")) {
		...
		if (cmd) {
			printf("Usage: xl [-v%s%s] %s %s\n\n%s.\n\n",
					cmd->modifies ? "f" : "", cmd->can_dryrun ? "N" : "",
...
static int def_getopt(int argc, char * const argv[], const char *optstring,
		const struct option *longopts, const char* helpstr, int reqargs) {
	int opt;
...
static int cd_insert(uint32_t domid, const char *virtdev, char *phys) {
	...
	xasprintf(&buf, "vdev=%s,access=r,devtype=cdrom,target=%s", virtdev,
			phys ? phys : "");
	...
	if (disk.format != LIBXL_DISK_FORMAT_EMPTY && stat(disk.pdev_path, &b)) {
	...
	out: libxl_device_disk_dispose(&disk);
	free(buf);
...
int main_usblist(int argc, char **argv) {
	...
	for (i = 0; i < numctrl; ++i) {
		...
		if (!libxl_device_usbctrl_getinfo(ctx, domid, &usbctrls[i],
				&usbctrlinfo)) {
...
int main_vncviewer(int argc, char **argv) {
	static const struct option opts[] = { { "autopass", 0, 0, 'a' }, {
...
static void pciassignable_list(void) {
	...
	if (pcidevs == NULL)
		return;
	for (i = 0; i < num; i++) {
		printf("%04x:%02x:%02x.%01x\n", pcidevs[i].domain, pcidevs[i].bus,
				pcidevs[i].dev, pcidevs[i].func);
		libxl_device_pci_dispose(&pcidevs[i]);
	}
	free(pcidevs);
}
...
static void destroy_domain(uint32_t domid, int force) {
	...
	if (rc) {
		fprintf(stderr, "failed to destroy domain %d (rc=%d)\n", domid, rc);
	}

#ifdef ENABLE_XS_SUB
        else{    //remove domain from cache
            pthread_mutex_lock(&lock_xssub);
            xssub_remove_domain(domid);
            pthread_mutex_unlock(&lock_xssub);
        }
#endif
...
#if defined(ENABLE_XS_SUB) && defined(ENABLE_XSSUB_VERBOSE)
        pthread_mutex_lock(&lock_xssub);
        printf("After (attempting to) destroying domain #%d...\n", domid);
        xssub_print_all_domains();
        pthread_mutex_unlock(&lock_xssub);
#endif
}
...
static void wait_for_domain_deaths(libxl_evgen_domain_death **deathws, int nr) {
	...
	while (1 && count < nr) {
		...
		case LIBXL_EVENT_TYPE_DOMAIN_DEATH:
			...
			break;
		case LIBXL_EVENT_TYPE_DOMAIN_SHUTDOWN:
			LOG("Domain %d has been shut down, reason code %d", event->domid,
					event->u.domain_shutdown.shutdown_reason);
			libxl_evdisable_domain_death(ctx, deathws[event->for_user]);
			count++;
			break;
...
static void shutdown_domain(uint32_t domid, libxl_evgen_domain_death **deathw,
...
static void list_domains_details(const libxl_dominfo *info, int nb_domain) {
	...
	out: if (default_output_format == OUTPUT_FORMAT_JSON) {
		yajl_gen_free(hand);
		if (s != yajl_gen_status_ok)
			fprintf(stderr,
...
bool cpupool, const libxl_dominfo *info, int nb_domain) {
	...
	for (i = 0; i < nb_domain; i++) {
		...
		printf("%-40s %5d %5lu %5d     %c%c%c%c%c%c  %8.1f", domname,
				info[i].domid,
				(unsigned long) ((info[i].current_memkb
						+ info[i].outstanding_memkb) / 1024),
				info[i].vcpu_online, info[i].running ? 'r' : '-',
				info[i].blocked ? 'b' : '-', info[i].paused ? 'p' : '-',
				info[i].shutdown ? 's' : '-',
				(shutdown_reason >= 0
						&& shutdown_reason
								< sizeof(shutdown_reason_letters) - 1 ?
						shutdown_reason_letters[shutdown_reason] : '?'),
				info[i].dying ? 'd' : '-', ((float) info[i].cpu_time / 1e9));
		free(domname);
		if (verbose) {
			printf(" " LIBXL_UUID_FMT, LIBXL_UUID_BYTES(info[i].uuid));
			if (info[i].shutdown)
				printf(" %8x", shutdown_reason);
			else
				printf(" %8s", "-");
		}
		if (claim)
...
static void core_dump_domain(uint32_t domid, const char *filename) {
	...
	if (rc) {
		fprintf(stderr, "core dump failed (rc=%d)\n", rc);
		exit(EXIT_FAILURE);
	}
}
...
static void save_domain_core_begin(uint32_t domid,
		const char *override_config_file, uint8_t **config_data_r,
		int *config_len_r) {
	int rc;
	libxl_domain_config d_config;
	char *config_c = 0;
	...
	if (override_config_file) {
		void *config_v = 0;
		rc = libxl_read_file_contents(ctx, override_config_file, &config_v,
				config_len_r);
		if (rc) {
...
#define ADD_OPTDATA(ptr, len) ({                                            \
	...
	CHK_ERRNOVAL(
			libxl_write_exactly(ctx, fd, &hdr, sizeof(hdr), source, "header"));
	CHK_ERRNOVAL(
			libxl_write_exactly(ctx, fd, optdata_begin, hdr.optional_data_len,
					source, "header"));
...
static int save_domain(uint32_t domid, const char *filename, int checkpoint,
	...
	save_domain_core_begin(domid, override_config_file, &config_data,
			&config_len);
...
static void migration_child_report(int recv_fd) {
	...
	if (!xl_child_pid(child_migration))
		return;
...
static void migrate_do_preamble(int send_fd, int recv_fd, pid_t child,
		uint8_t *config_data, int config_len, const char *rune) {
	int rc = 0;
	...
	rc = migrate_read_fixedmessage(recv_fd, migrate_receiver_banner,
			sizeof(migrate_receiver_banner) - 1, "banner", rune);
	if (rc) {
		close(send_fd);
	...
	save_domain_core_writeconfig(send_fd, "migration stream", config_data,
			config_len);
...
static void migrate_domain(uint32_t domid, const char *rune, int debug,
	...
	migrate_do_preamble(send_fd, recv_fd, child, config_data, config_len, rune);
	...
	rc = migrate_read_fixedmessage(recv_fd, migrate_receiver_ready,
			sizeof(migrate_receiver_ready), "ready message", rune);
	if (rc)
		goto failed_resume;
	...
	rc = migrate_read_fixedmessage(recv_fd, migrate_report,
			sizeof(migrate_report), "success/failure report message", rune);
	if (rc)
		goto failed_badly;

	rc = libxl_read_exactly(ctx, recv_fd, &rc_buf, 1, "migration ack stream",
	...
	if (rc_buf) {
		...
		rc = migrate_read_fixedmessage(recv_fd, migrate_permission_to_go,
				...
				"permission for sender to resume", rune);
		if (rc)
			goto failed_badly;
		...
		if (!rc)
			fprintf(stderr, "migration sender: Resumed OK.\n");
	...
	failed_suspend: close(send_fd);
	migration_child_report(recv_fd);
	fprintf(stderr, "Migration failed, failed to suspend at sender.\n");
	exit(EXIT_FAILURE);

	failed_resume: close(send_fd);
	migration_child_report(recv_fd);
	fprintf(stderr, "Migration failed, resuming at sender.\n");
	libxl_domain_resume(ctx, domid, 1, 0);
	exit(EXIT_FAILURE);

	failed_badly: fprintf(stderr,
...
static void migrate_receive(int debug, int daemonize, int monitor, int send_fd,
		int recv_fd, libxl_checkpointed_stream checkpointed,
		char *colo_proxy_script) {
	uint32_t domid;
	int rc, rc2;
	char rc_buf;
	...
	const char *ha =
			checkpointed == LIBXL_CHECKPOINTED_STREAM_COLO ? "COLO" : "Remus";
	...
	CHK_ERRNOVAL(
			libxl_write_exactly(ctx, send_fd, migrate_receiver_banner,
					sizeof(migrate_receiver_banner) - 1, "migration ack stream",
					"banner"));
	...
	rc = libxl_write_exactly(ctx, send_fd, migrate_receiver_ready,
			sizeof(migrate_receiver_ready), "migration ack stream",
			"ready message");
	if (rc)
		exit(EXIT_FAILURE);

	rc = migrate_read_fixedmessage(recv_fd, migrate_permission_to_go,
			sizeof(migrate_permission_to_go), "GO message", 0);
	if (rc)
		goto perhaps_destroy_notify_rc;
	...
	perhaps_destroy_notify_rc: rc2 = libxl_write_exactly(ctx, send_fd,
			migrate_report, sizeof(migrate_report), "migration ack stream",
	...
	rc_buf = -rc;
	assert(!!rc_buf == !!rc);
	rc2 = libxl_write_exactly(ctx, send_fd, &rc_buf, 1, "migration ack stream",
	...
	if (rc) {
		...
		rc2 = libxl_write_exactly(ctx, send_fd, migrate_permission_to_go,
				sizeof(migrate_permission_to_go), "migration ack stream",
...
int main_restore(int argc, char **argv) {
	...
	struct domain_create dom_info;
	int paused = 0, debug = 0, daemonize = 1, monitor = 1, console_autoconnect =
			0, vnc = 0, vncautopass = 0;
	int opt, rc;
	static struct option opts[] = { { "vncviewer", 0, 0, 'V' }, {
			"vncviewer-autopass", 0, 0, 'A' },
	COMMON_LONG_OPTS };
...
int main_migrate_receive(int argc, char **argv) {
	...
	int opt;
	char *script = NULL;
	static struct option opts[] = { { "colo", 0, 0, 0x100 },
	...
	STDOUT_FILENO, STDIN_FILENO, checkpointed, script);
...
int main_migrate(int argc, char **argv) {
	...
	static struct option opts[] = { { "debug", 0, 0, 0x100 }, { "live", 0, 0,
			0x200 },
	COMMON_LONG_OPTS };
	...
	if (!ssh_command[0]) {
		...
		xasprintf(&rune, "exec %s %s xl%s%.*s migrate-receive%s%s", ssh_command,
				host, pass_tty_arg ? " -t" : "", verbose_len, verbose_buf,
				daemonize ? "" : " -e", debug ? " -d" : "");
	}
...
int main_destroy(int argc, char **argv) {
	...
	int force = 0;
#ifdef DOM0_RECLAIM
	static unsigned int reclaim_memkb = 0;
#endif
	...
	if (domid == UINT_MAX)
		...
		destroy_domain(domid, force);

#ifdef DOM0_RECLAIM
		pthread_mutex_lock(&reclaim_lock);
		libxl_get_free_memory(ctx, &reclaim_memkb);
		if (reclaim_memkb) {
			libxl_set_memory_target(ctx, 0, reclaim_memkb, 1, 0);
			libxl_wait_for_memory_target(ctx, 0, 10, false);
		}
		pthread_mutex_unlock(&reclaim_lock);
#endif
...
static int main_shutdown_or_reboot(int do_reboot, int argc, char **argv) {
	...
	void (*fn)(uint32_t domid, libxl_evgen_domain_death **, libxl_ev_user,
			int) =
			do_reboot ? &reboot_domain : &shutdown_domain;
	int opt, i, nb_domain;
	int wait_for_it = 0, all = 0, nrdeathws = 0;
	int fallback_trigger = 0;
	static struct option opts[] = { { "all", 0, 0, 'a' }, { "wait", 0, 0, 'w' },
	COMMON_LONG_OPTS };
...
int main_list(int argc, char **argv) {
	...
	bool cpupool = false;
	bool numa = false;
	static struct option opts[] = { { "long", 0, 0, 'l' }, { "verbose", 0, 0,
			'v' }, { "context", 0, 0, 'Z' }, { "cpupool", 0, 0, 'c' }, { "numa",
			0, 0, 'n' },
	COMMON_LONG_OPTS };
	...
	if (details)
		...
		list_domains(verbose, context, false /* claim */, numa, cpupool, info,
				nb_domain);
...
int main_create(int argc, char **argv) {
	...
	int opt, rc;
	static struct option opts[] = { { "dryrun", 0, 0, 'n' }, { "quiet", 0, 0,
			'q' }, { "defconfig", 1, 0, 'f' }, { "vncviewer", 0, 0, 'V' }, {
			"vncviewer-autopass", 0, 0, 'A' },
	COMMON_LONG_OPTS };

	dom_info.extra_config = NULL;
	log_time("Enter main_create");
...
#ifdef XL_FORCE_NO_DAEMON
	daemonize = monitor = 0;
#endif /* XL_FORCE_NO_DAEMON */
	...
	log_time("Enter create_domain");
	rc = create_domain(&dom_info);
	log_time("Exit create_domain");
	if (rc < 0) {
		free(dom_info.extra_config);
		return -rc;
	}
	...
	free(dom_info.extra_config);
	log_time("Exit main_create");
	return 0;
}
/*
 int build_domain_to_create(int argc, char **argv)
 {
 const char *filename = NULL;
 struct domain_create dom_info;
 int paused = 0, debug = 0, daemonize = 1, console_autoconnect = 0,
...
/*int nr_child;
 int sig_Handler_main_create_terminated()
 {
 int status;
 int pid;
 pid = wait(&status);
...
/*
 int main_create_multiple(int argc, char **argv)
 {
 ...
 {
 cmd[i-1]=(char**)malloc(2*sizeof(char*));
 cmd[i-1][0]=(char*)malloc(7*sizeof(char));
 cmd[i-1][1]=(char*)malloc(strlen(argv[i])+1*sizeof(char));
 bzero(cmd[i-1][0],7);
 bzero(cmd[i-1][1],strlen(argv[i])+1);
 strcat(cmd[i-1][0],"create");
 strcat(cmd[i-1][1],argv[i]);
 }
 log_time("-----------------------------Enter main_create_multiple");
 ...
 nr_child=argc-1;
 fprintf(stderr,"xxxxxxxxxxxxxxxxxxxxxxxxxxxxx %d.\n",nr_child);
 while(nr_child>0)
 {
 pid=waitpid(-1, &status,WUNTRACED | WCONTINUED | WNOHANG);
 if (WIFSIGNALED(status))
 {
 fprintf(stdout,"uuuuuuuuuuuuuuuuuu Recu un kill %d \n", WTERMSIG(status));
 }else if (WIFSTOPPED(status))
 {
 fprintf(stdout,"uuuuuuuuuuuuuuuuuu Fils interrompu par un signal %d \n",WSTOPSIG(status));
 }else if (WIFEXITED(status))
 {
 fprintf(stdout,"uuuuuuuuuuuuuuuuuu  %d \n",WIFEXITED(status));
 nr_child--;
 }
 }
 out:
...
int main_config_update(int argc, char **argv) {
	...
	int opt, rc;
	int debug = 0;
	static struct option opts[] = { { "defconfig", 1, 0, 'f' },
	COMMON_LONG_OPTS };
	...
	if (!dryrun_only) {
		...
		rc = libxl_userdata_store(ctx, domid, "xl", config_data, config_len);
		if (rc) {
...
static void print_vcpuinfo(uint32_t tdomid, const libxl_vcpuinfo *vcpuinfo,
		uint32_t nr_cpus) {
	char *domname;

	/*      NAME  ID  VCPU */
	domname = libxl_domid_to_name(ctx, tdomid);
	printf("%-32s %5u %5u", domname, tdomid, vcpuinfo->vcpuid);
	free(domname);
	if (!vcpuinfo->online) {
		/*      CPU STA */
		printf("%5c %3c%cp ", '-', '-', '-');
		...
		/*      CPU STA */
		printf("%5u %3c%c- ", vcpuinfo->cpu, vcpuinfo->running ? 'r' : '-',
				vcpuinfo->blocked ? 'b' : '-');
	}
	/*      TIM */
...
static void vcpulist(int argc, char **argv) {
	...
	vcpulist_out: libxl_physinfo_dispose(&physinfo);
}
...
int main_vcpupin(int argc, char **argv) {
	static struct option opts[] = { { "force", 0, 0, 'f' },
	COMMON_LONG_OPTS };
	libxl_vcpuinfo *vcpuinfo;
	...
	rc = EXIT_SUCCESS;
	out: libxl_bitmap_dispose(&cpumap_soft);
...
int main_vcpuset(int argc, char **argv) {
	static struct option opts[] = { { "ignore-host", 0, 0, 'i' },
	COMMON_LONG_OPTS };
	...
	for (i = 0; i < nr; i++) {
		if (info[i].size != LIBXL_NUMAINFO_INVALID_ENTRY) {
			printf("%4d:    %6"PRIu64"     %6"PRIu64"      %d", i,
					info[i].size >> 20, info[i].free >> 20, info[i].dists[0]);
...
static void output_topologyinfo(void) {
	...
	for (i = 0; i < nr; i++) {
		if (cpuinfo[i].core != LIBXL_CPUTOPOLOGY_INVALID_ENTRY)
			printf("%3d:    %4d     %4d     %4d\n", i, cpuinfo[i].core,
					cpuinfo[i].socket, cpuinfo[i].node);
	}
	...
	for (i = 0; i < nr; i++) {
		if (pciinfo[i].node != LIBXL_PCITOPOLOGY_INVALID_ENTRY) {
			printf("%04x:%02x:%02x.%01x      %d\n", pciinfo[i].seg,
					pciinfo[i].bus, ((pciinfo[i].devfn >> 3) & 0x1f),
					(pciinfo[i].devfn & 7), pciinfo[i].node);
			valid_devs++;
		}
	}
...
int main_info(int argc, char **argv) {
	int opt;
	static struct option opts[] = { { "numa", 0, 0, 'n' },
	COMMON_LONG_OPTS };
	int numa = 0;
...
static int sched_credit_domain_output(int domid) {
	...
	printf("%-33s %4d %6d %4d\n", domname, domid, scinfo.weight, scinfo.cap);
	free(domname);
	libxl_domain_sched_params_dispose(&scinfo);
	return 0;
}
...
static int sched_credit_pool_output(uint32_t poolid) {
	...
	if (sched_credit_params_get(poolid, &scparam)) {
		...
		printf("Cpupool %s: tslice=%dms ratelimit=%dus\n", poolname,
				scparam.tslice_ms, scparam.ratelimit_us);
	}
	free(poolname);
	return 0;
}
...
static int sched_credit2_domain_output(int domid) {
	...
	domname = libxl_domid_to_name(ctx, domid);
	printf("%-33s %4d %6d\n", domname, domid, scinfo.weight);
	free(domname);
	libxl_domain_sched_params_dispose(&scinfo);
	return 0;
}

static int sched_rtds_domain_output(int domid) {
	char *domname;
	libxl_domain_sched_params scinfo;
	...
	domname = libxl_domid_to_name(ctx, domid);
	printf("%-33s %4d %9d %9d\n", domname, domid, scinfo.period, scinfo.budget);
	free(domname);
	libxl_domain_sched_params_dispose(&scinfo);
	return 0;
}
...
static int sched_rtds_vcpu_output(int domid, libxl_vcpu_sched_params *scinfo) {
	...
	for (i = 0; i < scinfo->num_vcpus; i++) {
		printf("%-33s %4d %4d %9"PRIu32" %9"PRIu32"\n", domname, domid,
				scinfo->vcpus[i].vcpuid, scinfo->vcpus[i].period,
...
int main_sched_credit(int argc, char **argv) {
	...
	bool opt_s = false;
	int opt, rc;
	static struct option opts[] = { { "domain", 1, 0, 'd' }, { "weight", 1, 0,
			'w' }, { "cap", 1, 0, 'c' }, { "schedparam", 0, 0, 's' }, {
			"tslice_ms", 1, 0, 't' }, { "ratelimit_us", 1, 0, 'r' }, {
			"cpupool", 1, 0, 'p' },
	COMMON_LONG_OPTS };
	...
	if (opt_s) {
		...
		if (cpupool) {
			if (libxl_cpupool_qualifier_to_cpupoolid(ctx, cpupool, &poolid,
			NULL) || !libxl_cpupoolid_is_valid(ctx, poolid)) {
				fprintf(stderr, "unknown cpupool \'%s\'\n", cpupool);
				return EXIT_FAILURE;
			}
		}
		...
		if (sched_domain_output(LIBXL_SCHEDULER_CREDIT,
				sched_credit_domain_output, sched_credit_pool_output, cpupool))
			return EXIT_FAILURE;
	} else {
...
int main_sched_credit2(int argc, char **argv) {
	...
	if (!dom) { /* list all domain's credit scheduler info */
		if (sched_domain_output(LIBXL_SCHEDULER_CREDIT2,
				sched_credit2_domain_output, sched_default_pool_output,
				cpupool))
			return EXIT_FAILURE;
	} else {
		uint32_t domid = find_domain(dom);
...
int main_sched_rtds(int argc, char **argv) {
	...
	int opt, i, rc, r;
	static struct option opts[] = { { "domain", 1, 0, 'd' }, { "period", 1, 0,
			'p' }, { "budget", 1, 0, 'b' }, { "vcpuid", 1, 0, 'v' }, {
			"cpupool", 1, 0, 'c' },
	COMMON_LONG_OPTS };
	...
	if ((!dom) && opt_all) {
		...
		rc = -sched_vcpu_output(LIBXL_SCHEDULER_RTDS,
				sched_rtds_vcpu_output_all, sched_rtds_pool_output, cpupool);
		if (rc) {
			r = EXIT_FAILURE;
			goto out;
		}
		...
		rc = -sched_domain_output(LIBXL_SCHEDULER_RTDS,
				sched_rtds_domain_output, sched_rtds_pool_output, cpupool);
		if (rc) {
			r = EXIT_FAILURE;
			goto out;
		}
		...
		if (!opt_v && !opt_all) { /* output default scheduling parameters */
			...
			if (v_index > 0) {
				scinfo.vcpus = (libxl_sched_params *) xmalloc(
						sizeof(libxl_sched_params) * (v_index));
				for (i = 0; i < v_index; i++)
					scinfo.vcpus[i].vcpuid = vcpus[i];
				rc = -sched_rtds_vcpu_output(domid, &scinfo);
			} else
				/* get params for all vcpus */
...
int main_domid(int argc, char **argv) {
	...
	if (libxl_name_to_domid(ctx, domname, &domid)) {
		fprintf(stderr,
				"Can't get domid of domain name '%s', maybe this domain does not exist.\n",
				domname);
		return EXIT_FAILURE;
	}
...
int main_domname(int argc, char **argv) {
	...
	if (!domname) {
		fprintf(stderr,
				"Can't get domain name of domain id '%d', maybe this domain does not exist.\n",
				domid);
		return EXIT_FAILURE;
	}
...
int main_networklist(int argc, char **argv) {
	...
	printf("%-3s %-2s %-17s %-6s %-5s %-6s %5s/%-5s %-30s\n", "Idx", "BE",
			"Mac Addr.", "handle", "state", "evt-ch", "tx-", "rx-ring-ref",
			"BE-path");
		...
		for (i = 0; i < nb; ++i) {
			if (!libxl_device_nic_getinfo(ctx, domid, &nics[i], &nicinfo)) {
				...
				printf("%6d %5d %6d %5d/%-11d %-30s\n", nicinfo.devid,
						nicinfo.state, nicinfo.evtch, nicinfo.rref_tx,
						nicinfo.rref_rx, nicinfo.backend);
...
int main_networkdetach(int argc, char **argv) {
	...
	if (!strchr(argv[optind + 1], ':')) {
		if (libxl_devid_to_device_nic(ctx, domid, atoi(argv[optind + 1]),
				&nic)) {
			fprintf(stderr, "Unknown device %s.\n", argv[optind + 1]);
			return 1;
		}
	...
	for (argv += optind, argc -= optind; argc > 0; --argc, ++argv) {
		...
		for (i = 0; i < nb; ++i) {
			if (!libxl_device_channel_getinfo(ctx, domid, &channels[i],
				...
				printf("%-10s ",
						libxl_channel_connection_to_string(
...
int main_blockattach(int argc, char **argv) {
	...
	parse_disk_config_multistring(&config, argc - optind,
			(const char* const *) argv + optind, &disk);
		...
		for (i = 0; i < nb; i++) {
			if (!libxl_device_disk_getinfo(ctx, domid, &disks[i], &diskinfo)) {
				...
				printf("%-5d %-3d %-6d %-5d %-6d %-8d %-30s\n", diskinfo.devid,
						diskinfo.backend_id, diskinfo.frontend_id,
...
int main_vtpmlist(int argc, char **argv) {
	...
	printf("%-3s %-2s %-36s %-6s %-5s %-6s %-5s %-10s\n", "Idx", "BE", "Uuid",
			"handle", "state", "evt-ch", "ring-ref", "BE-path");
	for (argv += optind, argc -= optind; argc > 0; --argc, ++argv) {
		uint32_t domid;
		if (libxl_domain_qualifier_to_domid(ctx, *argv, &domid) < 0) {
			fprintf(stderr, "%s is an invalid domain identifier\n", *argv);
		...
		for (i = 0; i < nb; ++i) {
			if (!libxl_device_vtpm_getinfo(ctx, domid, &vtpms[i], &vtpminfo)) {
				...
				printf("%-3d %-2d " LIBXL_UUID_FMT " %6d %5d %6d %8d %-30s\n",
						...
						LIBXL_UUID_BYTES(vtpminfo.uuid), vtpminfo.devid,
						vtpminfo.state, vtpminfo.evtch, vtpminfo.rref,
						vtpminfo.backend);
...
static void print_dom0_uptime(int short_mode, time_t now) {
	...
	if (short_mode) {
		...
		printf(" %s up %s, %s (%d)\n", now_str, uptime_str, domname, 0);
	} else {
		now_str = NULL;
		uptime_str = uptime_to_string(uptime, 0);
		printf("%-33s %4d %s\n", domname, 0, uptime_str);
	}
	...
	free(domname);
	return;
	err: fprintf(stderr, "Can not get Dom0 uptime.\n");
	exit(-1);
}
...
static void print_domU_uptime(uint32_t domuid, int short_mode, time_t now) {
	...
	if (short_mode) {
		now_str = current_time_to_string(now);
		uptime_str = uptime_to_string(uptime, 1);
		printf(" %s up %s, %s (%d)\n", now_str, uptime_str, domname, domuid);
	} else {
		now_str = NULL;
		uptime_str = uptime_to_string(uptime, 0);
		printf("%-33s %4d %s\n", domname, domuid, uptime_str);
	}
...
int main_cpupoolcreate(int argc, char **argv) {
	...
	char *extra_config = NULL;
	int opt;
	static struct option opts[] = { { "defconfig", 1, 0, 'f' }, { "dryrun", 0,
			0, 'n' },
	COMMON_LONG_OPTS };
	int ret;
	char *config_data = 0;
	...
	if (!xlu_cfg_get_list(config, "nodes", &nodes, 0, 0)) {
		...
		while ((buf = xlu_cfg_get_listitem(nodes, n_nodes)) != NULL) {
			n = atoi(buf);
			for (i = 0; i < nr; i++) {
				if ((topology[i].node == n) && libxl_bitmap_test(&freemap, i)) {
					libxl_bitmap_set(&cpumap, i);
					n_cpus++;
				}
			}
	...
	out_cfg: xlu_cfg_destroy(config);
	out: libxl_bitmap_dispose(&freemap);
	libxl_bitmap_dispose(&cpumap);
	free(name);
	free(config_data);
	free(extra_config);
...
int main_cpupoollist(int argc, char **argv) {
	int opt;
	static struct option opts[] = { { "cpus", 0, 0, 'c' },
	COMMON_LONG_OPTS };
	int opt_cpus = 0;
...
int main_cpupoolcpuadd(int argc, char **argv) {
	...
	out: libxl_bitmap_dispose(&cpumap);
	return rc;
}
...
int main_cpupoolcpuremove(int argc, char **argv) {
	...
	if (libxl_cpupool_cpuremove_cpumap(ctx, poolid, &cpumap)) {
		fprintf(stderr,
				"Some cpus may have not or only partially been removed from '%s'.\n",
				pool);
		fprintf(stderr,
				"If a cpu can't be added to another cpupool, add it to '%s' again and retry.\n",
				pool);
	}
...
int main_cpupoolnumasplit(int argc, char **argv) {
	...
	out: libxl_cputopology_list_free(topology, n_cpus);
	libxl_bitmap_dispose(&cpumap);
	libxl_dominfo_dispose(&info);
	free(name);
		...
		if (!ssh_command[0]) {
			...
			if (!libxl_defbool_val(r_info.colo)) {
				xasprintf(&rune, "exec %s %s xl migrate-receive %s %s",
						ssh_command, host, "-r", daemonize ? "" : " -e");
			} else {
				xasprintf(&rune, "exec %s %s xl migrate-receive %s %s %s %s",
						ssh_command, host, "--colo",
...
int main_devd(int argc, char **argv) {
	...
	static const struct option opts[] = { { "pidfile", 1, 0, 'p' },
	COMMON_LONG_OPTS, { 0, 0, 0, 0 } };
...
static int psr_cmt_get_mem_bandwidth(uint32_t domid, libxl_psr_cmt_type type,
		uint32_t socketid, uint64_t *bandwidth_r) {
	...
	while (1) {
		rc = libxl_psr_cmt_get_sample(ctx, domid, type, socketid, &sample1,
				&tsc1);
		if (rc < 0)
			return rc;
		...
		rc = libxl_psr_cmt_get_sample(ctx, domid, type, socketid, &sample2,
				&tsc2);
		if (rc < 0)
			return rc;
...
static void psr_cmt_print_domain_info(libxl_dominfo *dominfo,
		libxl_psr_cmt_type type, libxl_bitmap *socketmap) {
	char *domain_name;
	uint32_t socketid;
...
static int psr_cmt_show(libxl_psr_cmt_type type, uint32_t domid) {
	...
	out: libxl_bitmap_dispose(&socketmap);
	return rc;
}
...
static int psr_cat_hwinfo(void) {
	...
	for (i = 0; i < nr; i++) {
		...
		printf("%-16s: %#llx\n", "Default CBM", (1ull << info[i].cbm_len) - 1);
	}

	out: libxl_psr_cat_info_list_free(info, nr);
	return rc;
}
...
int main_psr_cat_cbm_set(int argc, char **argv) {
	...
	static struct option opts[] = { { "socket", 1, 0, 's' },
			{ "data", 0, 0, 'd' }, { "code", 0, 0, 'c' },
			COMMON_LONG_OPTS };
...
int main_psr_hwinfo(int argc, char **argv) {
	...
	static struct option opts[] = { { "cmt", 0, 0, 'm' }, { "cat", 0, 0, 'a' },
	COMMON_LONG_OPTS };
