...
static const libxl__checkpoint_device_instance_ops *colo_ops[] = {
		&colo_save_device_nic, &colo_save_device_qdisk,
		NULL, };
...
void libxl__colo_save_setup(libxl__egc *egc, libxl__colo_save_state *css) {
	...
	out: dss->callback(egc, dss, ERROR_FAIL);
}
...
void libxl__colo_save_teardown(libxl__egc *egc, libxl__colo_save_state *css,
		int rc) {
	libxl__domain_save_state *dss = CONTAINER_OF(css, *dss, css);
...
static void colo_read_svm_suspended_done(libxl__egc *egc,
		libxl__colo_save_state *css, int id);
/*
...
static void colo_postsuspend_cb(libxl__egc *egc,
	...
	out: libxl__xc_domain_saverestore_async_callback_done(egc, &dss->sws.shs,
			!rc);
}
...
static void colo_proxy_async_call_done(libxl__egc *egc, libxl__ev_child *child,
		int pid, int status);
...
static void colo_proxy_wait_for_checkpoint(libxl__egc *egc,
	...
	ASYNC_CALL(egc, dss->cds.ao, &css->child, css,
			colo_proxy_async_wait_for_checkpoint, colo_proxy_async_call_done);
}
