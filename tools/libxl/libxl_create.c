...
int libxl__domain_build_info_setdefault(libxl__gc *gc,
	...
	if (b_info->type == LIBXL_DOMAIN_TYPE_HVM
		...
		LOG(ERROR, "device model stubdomains require \"qemu-xen-traditional\"");
		return ERROR_INVAL;
	}
	...
	if (b_info->cpumap.size && !b_info->num_vcpu_hard_affinity) {
		...
		for (i = 0; i < b_info->max_vcpus; i++) {
			if (libxl_cpu_bitmap_alloc(CTX, &b_info->vcpu_hard_affinity[i],
					0)) {
	...
	case LIBXL_DOMAIN_TYPE_HVM:
		...
		case LIBXL_DEVICE_MODEL_VERSION_QEMU_XEN_TRADITIONAL:
			...
			case LIBXL_VGA_INTERFACE_TYPE_STD:
				...
				if (b_info->video_memkb < 8 * 1024) {
					LOG(ERROR,
							"videoram must be at least 8 MB for STDVGA on QEMU_XEN_TRADITIONAL");
					return ERROR_INVAL;
				}
				break;
			...
			default:
				...
				if (b_info->video_memkb != 4 * 1024)
					LOG(WARN,
							"ignoring videoram other than 4 MB for CIRRUS on QEMU_XEN_TRADITIONAL");
				break;
			}
			break;
		case LIBXL_DEVICE_MODEL_VERSION_NONE:
		...
		default:
			...
			default:
				...
				if (b_info->video_memkb < 8 * 1024) {
					LOG(ERROR,
							"videoram must be at least 8 MB for CIRRUS on QEMU_XEN");
					return ERROR_INVAL;
				}
				break;
			}
...
static void init_console_info(libxl__gc *gc, libxl__device_console *console,
		int dev_num) {
...
int libxl__domain_build(libxl__gc *gc, libxl_domain_config *d_config,
	...
	case LIBXL_DOMAIN_TYPE_HVM:
		...
		vments[3] = "hvm";
		vments[4] = "start_time";
		vments[5] = GCSPRINTF("%lu.%02d", start_time.tv_sec,
	...
	case LIBXL_DOMAIN_TYPE_PV:
		...
		vments[i++] = "start_time";
		vments[i++] = GCSPRINTF("%lu.%02d", start_time.tv_sec,
				(int )start_time.tv_usec / 10000);
...
int libxl__domain_make(libxl__gc *gc, libxl_domain_config *d_config,
	...
	log_time("Enter libxl__uuid2string");
	uuid_string = libxl__uuid2string(gc, info->uuid);
	log_time("Exit libxl__uuid2string");
	if (!uuid_string) {
		rc = ERROR_NOMEM;
		goto out;
	}
	...
	flags = 0;
	log_time("Enter libxl_defbool_val");
	...
	log_time("Exit libxl_defbool_val");

	/* Ultimately, handle is an array of 16 uint8_t, same as uuid */
	libxl_uuid_copy(ctx, (libxl_uuid *) handle, &info->uuid);

	log_time("Enter libxl__arch_domain_prepare_config");
	ret = libxl__arch_domain_prepare_config(gc, d_config, xc_config);
	log_time("Exit libxl__arch_domain_prepare_config");
	if (ret < 0) {
	...
	if (!libxl_domid_valid_guest(*domid)) {
		log_time("Enter xc_domain_create");
		ret = xc_domain_create(ctx->xch, info->ssidref, handle, flags, domid,
				xc_config);
		log_time("Exit xc_domain_create");
		if (ret < 0) {
	...
	log_time("Enter libxl__arch_domain_save_config");
	rc = libxl__arch_domain_save_config(gc, d_config, xc_config);
	log_time("Exit libxl__arch_domain_save_config");
	if (rc < 0)
		goto out;

	log_time("Enter xc_cpupool_movedomain");
	ret = xc_cpupool_movedomain(ctx->xch, info->poolid, *domid);
	log_time("Exit xc_cpupool_movedomain");
	if (ret < 0) {
	...
	log_time("Enter libxl__xs_get_dompath");
	dom_path = libxl__xs_get_dompath(gc, *domid);
	log_time("Exit libxl__xs_get_dompath");
	if (!dom_path) {
		rc = ERROR_FAIL;
		goto out;
	}

	log_time("Enter GCSPRINTF");
	vm_path = GCSPRINTF("/vm/%s", uuid_string);
	log_time("Exit GCSPRINTF");
	if (!vm_path) {
	...
	log_time("Enter libxl__xs_libxl_path");
	libxl_path = libxl__xs_libxl_path(gc, *domid);
	log_time("Exit libxl__xs_libxl_path");
	if (!libxl_path) {
		rc = ERROR_FAIL;
		goto out;
	}
	...
	log_time("Enter retry_transaction");
	...
	libxl__xs_mknod(gc, t, GCSPRINTF("%s/device", libxl_path), noperm,
			ARRAY_SIZE(noperm));
	...
	libxl__xs_mknod(gc, t, GCSPRINTF("%s/cpu", dom_path), roperm,
			ARRAY_SIZE(roperm));
	libxl__xs_mknod(gc, t, GCSPRINTF("%s/memory", dom_path), roperm,
			ARRAY_SIZE(roperm));
	libxl__xs_mknod(gc, t, GCSPRINTF("%s/device", dom_path), roperm,
			ARRAY_SIZE(roperm));
	libxl__xs_mknod(gc, t, GCSPRINTF("%s/control", dom_path), roperm,
			ARRAY_SIZE(roperm));
	if (info->type == LIBXL_DOMAIN_TYPE_HVM)
		libxl__xs_mknod(gc, t, GCSPRINTF("%s/hvmloader", dom_path), roperm,
				ARRAY_SIZE(roperm));

	libxl__xs_mknod(gc, t, GCSPRINTF("%s/control/shutdown", dom_path), rwperm,
			ARRAY_SIZE(rwperm));
	libxl__xs_mknod(gc, t,
			GCSPRINTF("%s/device/suspend/event-channel", dom_path), rwperm,
			ARRAY_SIZE(rwperm));
	libxl__xs_mknod(gc, t, GCSPRINTF("%s/data", dom_path), rwperm,
			ARRAY_SIZE(rwperm));
	libxl__xs_mknod(gc, t, GCSPRINTF("%s/drivers", dom_path), rwperm,
			ARRAY_SIZE(rwperm));
	libxl__xs_mknod(gc, t, GCSPRINTF("%s/feature", dom_path), rwperm,
			ARRAY_SIZE(rwperm));
	libxl__xs_mknod(gc, t, GCSPRINTF("%s/attr", dom_path), rwperm,
			ARRAY_SIZE(rwperm));
	...
	xs_write(ctx->xsh, t,
			GCSPRINTF("%s/control/platform-feature-multiprocessor-suspend",
					dom_path), "1", 1);
	xs_write(ctx->xsh, t,
			GCSPRINTF("%s/control/platform-feature-xs_reset_watches", dom_path),
			"1", 1);
	...
	t = 0;
	log_time("Exit retry_transaction");
...
static void domcreate_stream_done(libxl__egc *egc,
		libxl__stream_read_state *srs, int ret);

static void domcreate_rebuild_done(libxl__egc *egc,
		libxl__domain_create_state *dcs, int ret);

/* Our own function to clean up and call the user's callback.
 * The final call in the sequence. */
static void domcreate_complete(libxl__egc *egc, libxl__domain_create_state *dcs,
		int rc);
...
static void initiate_domain_create(libxl__egc *egc,
	 ...
	 */
	if (d_config->c_info.type == LIBXL_DOMAIN_TYPE_HVM && d_config->num_pcidevs
			&& pod_enabled) {
		ret = ERROR_INVAL;
		LOG(ERROR,
	...
	log_time("Enter libxl__domain_create_info_setdefault");
	ret = libxl__domain_create_info_setdefault(gc, &d_config->c_info);
	log_time("Exit libxl__domain_create_info_setdefault");
	if (ret) {
		LOG(ERROR, "Unable to set domain create info defaults");
		goto error_out;
	}

	log_time("Enter libxl__domain_make");
	ret = libxl__domain_make(gc, d_config, &domid, &state->config); 
	log_time("Exit libxl__domain_make");
	...
	log_time("Enter libxl__domain_build_info_setdefault");
	ret = libxl__domain_build_info_setdefault(gc, &d_config->b_info);
	log_time("Exit libxl__domain_build_info_setdefault");
	if (ret) {
	...
	if (restore_fd >= 0 || dcs->domid_soft_reset != INVALID_DOMID) {
		...
		log_time("Enter libxl__bootloader_run");
		libxl__bootloader_run(egc, &dcs->bl);
		log_time("Exit libxl__bootloader_run");
	}
	return;
...
static void domcreate_console_available(libxl__egc *egc,
	...
	libxl__ao_progress_report(egc, dcs->ao, &dcs->aop_console_how,
			NEW_EVENT(egc, DOMAIN_CREATE_CONSOLE_AVAILABLE, dcs->guest_domid,
...
static void domcreate_bootloader_done(libxl__egc *egc,
	...
	out: domcreate_stream_done(egc, &dcs->srs, rc);
}
...
static void domcreate_stream_done(libxl__egc *egc,
	...
	if (info->type == LIBXL_DOMAIN_TYPE_HVM) {
		state->saved_state = GCSPRINTF(LIBXL_DEVICE_MODEL_RESTORE_FILE".%d",
				domid);
	}

	out: if (info->type == LIBXL_DOMAIN_TYPE_PV) {
...
static void domcreate_launch_dm(libxl__egc *egc, libxl__multidev *multidev,
	...
	for (i = 0; i < d_config->b_info.num_ioports; i++) {
		...
		LOG(DEBUG, "dom%d ioports %"PRIx32"-%"PRIx32, domid, io->first,
				io->first + io->number - 1);

		ret = xc_domain_ioport_permission(CTX->xch, domid, io->first,
				io->number, 1);
		if (ret < 0) {
			LOGE(ERROR, "failed give dom%d access to ioports %"PRIx32"-%"PRIx32,
	...
	for (i = 0; i < d_config->b_info.num_irqs; i++) {
		...
		ret = irq >= 0 ?
				libxl__arch_domain_map_irq(gc, domid, irq) : -EOVERFLOW;
		if (ret) {
	...
	for (i = 0; i < d_config->b_info.num_iomem; i++) {
		...
		LOG(DEBUG, "dom%d iomem %"PRIx64"-%"PRIx64, domid, io->start,
				io->start + io->number - 1);

		ret = xc_domain_iomem_permission(CTX->xch, domid, io->start, io->number,
				1);
		if (ret < 0) {
			LOGE(ERROR,
		...
		ret = xc_domain_memory_mapping(CTX->xch, domid, io->gfn, io->start,
				io->number, 1);
		if (ret < 0) {
			LOGE(ERROR,
					"failed to map to dom%d iomem range %"PRIx64"-%"PRIx64 " to guest address %"PRIx64,
...
static void domcreate_complete(libxl__egc *egc, libxl__domain_create_state *dcs,
	...
	if (retain_domain) {
		...
		if (!lock) {
			...
			int cfg_rc = libxl__set_domain_configuration(gc, dcs->guest_domid,
					d_config_saved);
			if (!rc)
				rc = cfg_rc;
...
static void domain_create_cb(libxl__egc *egc, libxl__domain_create_state *dcs,
		int rc, uint32_t domid);

static int do_domain_create(libxl_ctx *ctx, libxl_domain_config *d_config,
		uint32_t *domid, int restore_fd, int send_back_fd,
		const libxl_domain_restore_params *params,
		const char *colo_proxy_script, const libxl_asyncop_how *ao_how,
		const libxl_asyncprogress_how *aop_console_how) {
	VLAD_AO_CREATE(ctx, 0, ao_how);
	libxl__app_domain_create_state *cdcs;
	int rc;
	...
	if (restore_fd > -1) {
		...
		rc = libxl__fd_flags_modify_save(gc, cdcs->dcs.restore_fd,
				~(O_NONBLOCK | O_NDELAY), 0, &cdcs->dcs.restore_fdfl);
		if (rc < 0)
			goto out_err;
	}
	...
	log_time("Enter initiate_domain_create");
	initiate_domain_create(egc, &cdcs->dcs);
	log_time("Exit initiate_domain_create");
...
static void domain_soft_reset_cb(libxl__egc *egc,
	...
	error: domcreate_complete(egc, &cdcs->dcs, rc);
}

static int do_domain_soft_reset(libxl_ctx *ctx, libxl_domain_config *d_config,
		uint32_t domid_soft_reset, const libxl_asyncop_how *ao_how,
		const libxl_asyncprogress_how *aop_console_how) {
...
static void domain_create_cb(libxl__egc *egc, libxl__domain_create_state *dcs,
	...
	*cdcs->domid_out = domid;
	...
	if (dcs->restore_fd > -1) {
		flrc = libxl__fd_flags_restore(gc, dcs->restore_fd, dcs->restore_fdfl);
		/*
...
int libxl_domain_create_new(libxl_ctx *ctx, libxl_domain_config *d_config,
	...
	return do_domain_create(ctx, d_config, domid, -1, -1, NULL, NULL, ao_how,
			aop_console_how);
}
