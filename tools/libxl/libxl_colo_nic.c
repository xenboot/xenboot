...
static void setup_async_exec(libxl__checkpoint_device *dev, char *op,
		libxl__colo_proxy_state *cps, int side, char *colo_proxy_script) {
	int arraysize, nr = 0;
...
const libxl__checkpoint_device_instance_ops colo_save_device_nic = { .kind =
		LIBXL__DEVICE_KIND_VIF, .setup = colo_nic_save_setup, .teardown =
		colo_nic_save_teardown, };
...
const libxl__checkpoint_device_instance_ops colo_restore_device_nic = { .kind =
		LIBXL__DEVICE_KIND_VIF, .setup = colo_nic_restore_setup, .teardown =
		colo_nic_restore_teardown, };
