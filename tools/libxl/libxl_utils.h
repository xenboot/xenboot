...
int libxl_cpupool_qualifier_to_cpupoolid(libxl_ctx *ctx, const char *p,
		uint32_t *poolid_r, int *was_name_r);
...
int libxl_string_to_backend(libxl_ctx *ctx, char *s,
...
int libxl_write_exactly(libxl_ctx *ctx, int fd, const void *data, ssize_t sz,
		const char *filename, const char *what);
...
int libxl_mac_to_device_nic(libxl_ctx *ctx, uint32_t domid, const char *mac,
...
int libxl_uuid_to_device_vtpm(libxl_ctx *ctx, uint32_t domid, libxl_uuid *uuid,
		libxl_device_vtpm *vtpm);
int libxl_devid_to_device_vtpm(libxl_ctx *ctx, uint32_t domid, int devid,
		libxl_device_vtpm *vtpm);
int libxl_devid_to_device_usbctrl(libxl_ctx *ctx, uint32_t domid, int devid,
		libxl_device_usbctrl *usbctrl);
int libxl_ctrlport_to_device_usbdev(libxl_ctx *ctx, uint32_t domid, int ctrl,
		int port, libxl_device_usbdev *usbdev);
...
static inline int libxl_bitmap_equal(const libxl_bitmap *ba,
		const libxl_bitmap *bb, int nr_bits) {
	int i;
...
static inline void log_time(char *msg) {
	...
	tid = gettid();
	sprintf(thread_log_filename, "logTh%d", tid);
	thread_log = fopen(thread_log_filename, "a");

	gettimeofday(&currentTime, NULL);
	fprintf(thread_log, "%ld xxx %s %ld\n", gettid(), msg,
...
int libxl_nodemap_to_cpumap(libxl_ctx *ctx, const libxl_bitmap *nodemap,
		libxl_bitmap *cpumap);
/* Populate cpumap with the cpus spanned by node */
int libxl_node_to_cpumap(libxl_ctx *ctx, int node, libxl_bitmap *cpumap);
/* Populate nodemap with the nodes of the cpus in cpumap */
int libxl_cpumap_to_nodemap(libxl_ctx *ctx, const libxl_bitmap *cpumap,
