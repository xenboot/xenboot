...
int libxl__xswait_start(libxl__gc *gc, libxl__xswait_state *xswa) {
	...
	err: libxl__xswait_stop(gc, xswa);
	return rc;
}
...
static void datacopier_writable(libxl__egc *egc, libxl__ev_fd *ev, int fd,
...
static int datacopier_pollhup_handled(libxl__egc *egc,
		libxl__datacopier_state *dc, int fd, short revents, int onwrite) {
	STATE_AO_GC(dc->ao);

	if (dc->callback_pollhup && (revents & POLLHUP)) {
		LOG(DEBUG, "received POLLHUP on fd %d: %s during copy of %s", fd,
...
static void datacopier_abort(libxl__egc *egc, libxl__ao_abortable *abrt, int rc) {
...
static void datacopier_readable(libxl__egc *egc, libxl__ev_fd *ev, int fd,
		short events, short revents) {
	...
	if (revents & ~(POLLIN | POLLHUP)) {
		LOG(ERROR, "unexpected poll event 0x%x on fd %d (expected POLLIN "
				"and/or POLLHUP) reading %s during copy of %s", revents, fd,
	...
	for (;;) {
		...
		if (r < 0) {
			...
			if (errno == EWOULDBLOCK) {
				if (revents & POLLHUP) {
					LOG(ERROR, "poll reported HUP but fd read gave EWOULDBLOCK"
							" on %s during copy of %s", dc->readwhat,
							dc->copywhat);
			...
			LOGE(ERROR, "error reading %s during copy of %s", dc->readwhat,
					dc->copywhat);
...
static void datacopier_writable(libxl__egc *egc, libxl__ev_fd *ev, int fd,
	...
	if (revents & ~POLLOUT) {
		LOG(ERROR, "unexpected poll event 0x%x on fd %d (should be POLLOUT)"
				" writing %s during copy of %s", revents, fd, dc->writewhat,
				dc->copywhat);
		datacopier_callback(egc, dc, ERROR_FAIL, -1, EIO);
		return;
	}
	...
	for (;;) {
		...
		if (r < 0) {
			...
			if (errno == EWOULDBLOCK)
				break;
			assert(errno);
			LOGE(ERROR, "error writing to %s during copy of %s", dc->writewhat,
					dc->copywhat);
...
static void openpty_cleanup(libxl__openpty_state *op) {
	...
	for (i = 0; i < op->count; i++) {
		...
		libxl__carefd_close(res->master);
		res->master = 0;
		libxl__carefd_close(res->slave);
		res->slave = 0;
	}
}

static void openpty_exited(libxl__egc *egc, libxl__ev_child *child, pid_t pid,
		int status) {
	...
	if (status) {
		...
		libxl_report_child_exitstatus(CTX,
				op->rc ? LIBXL__LOG_ERROR : LIBXL__LOG_WARNING, "openpty child",
				pid, status);
	}
	if (op->rc)
		openpty_cleanup(op);
...
int libxl__openptys(libxl__openpty_state *op, struct termios *termp,
	...
	if (!pid) {
		...
		for (i = 0; i < count; i++) {
			...
			if (r) {
				LOGE(ERROR, "openpty failed");
				_exit(-1);
			}
		}
		rc = libxl__sendmsg_fds(gc, sockets[1], "", 1, 2 * count, &ptyfds[0][0],
				"ptys");
		if (rc) {
			LOGE(ERROR, "sendmsg to parent failed");
			_exit(-1);
		}
		_exit(0);
	}
	...
	libxl__carefd_begin();
	char buf[1];
	rc = libxl__recvmsg_fds(gc, sockets[0], buf, 1, 2 * count, &ptyfds[0][0],
			"ptys");
	if (!rc) {
		for (i = 0; i < count; i++) {
			libxl__openpty_result *res = &op->results[i];
...
static void async_exec_timeout(libxl__egc *egc, libxl__ev_time *ev,
		const struct timeval *requested_abs, int rc) {
	...
	if (kill(aes->child.pid, SIGKILL)) {
		LOGEV(ERROR, errno, "unable to kill %s [%ld]", aes->what,
...
static void async_exec_done(libxl__egc *egc, libxl__ev_child *child, pid_t pid,
		int status) {
	...
	if (status) {
		if (!aes->rc)
			libxl_report_child_exitstatus(CTX, LIBXL__LOG_ERROR, aes->what, pid,
					status);
	}
...
int libxl__async_exec_start(libxl__async_exec_state *aes) {
	...
	if (libxl__ev_time_register_rel(ao, &aes->time, async_exec_timeout,
			aes->timeout_ms)) {
	...
	if (!pid) {
		/* child */
		libxl__exec(gc, aes->stdfds[0], aes->stdfds[1], aes->stdfds[2], args[0],
				args, aes->env);
	}
...
void libxl__kill(libxl__gc *gc, pid_t pid, int sig, const char *what) {
	int r = kill(pid, sig);
	if (r)
		LOGE(WARN, "failed to kill() %s [%lu] (signal %d)", what,
