...
int init_subkind_nic(libxl__checkpoint_devices_state *cds) {
	...
	if (ret) {
		LOG(ERROR, "failed to open netlink socket: %s", nl_geterror(ret));
		rc = ERROR_FAIL;
		goto out;
	}
	...
	if (ret) {
		LOG(ERROR, "failed to allocate qdisc cache: %s", nl_geterror(ret));
		rc = ERROR_FAIL;
		goto out;
	}
...
static const char *get_vifname(libxl__checkpoint_device *dev,
	...
	if (!rc && !vifname) {
		vifname = libxl__device_nic_devname(gc, domid, nic->devid,
				nic->nictype);
	}
...
static void setup_async_exec(libxl__checkpoint_device *dev, char *op) {
	...
	env[nr++] = "XENBUS_PATH";
	env[nr++] = GCSPRINTF("%s/remus/netbuf/%d", libxl__xs_libxl_path(gc, domid),
			dev_id);
...
static void netbuf_setup_script_cb(libxl__egc *egc,
	...
	rc = libxl__xs_read_checked(gc, XBT_NULL,
			GCSPRINTF("%s/remus/netbuf/%d/ifb", libxl__xs_libxl_path(gc, domid),
					devid), ifb);
	if (rc)
		goto out;

	if (!(*ifb)) {
		LOG(ERROR, "Cannot get ifb dev name for domain %u dev %s", domid, vif);
		rc = ERROR_FAIL;
		goto out;
	}
	...
	rc = libxl__xs_read_checked(gc, XBT_NULL,
			GCSPRINTF("%s/hotplug-error", out_path_base), &hotplug_error);
	if (rc)
		goto out;
...
enum {
	tc_buffer_start, tc_buffer_release
};

/* API implementations */

static int remus_netbuf_op(libxl__remus_device_nic *remus_nic,
		libxl__checkpoint_devices_state *cds, int buffer_op) {
	int rc, ret;
	libxl__remus_state *rs = cds->concrete_data;
...
const libxl__checkpoint_device_instance_ops remus_device_nic = { .kind =
		LIBXL__DEVICE_KIND_VIF, .setup = nic_setup, .teardown = nic_teardown,
		.postsuspend = nic_postsuspend, .commit = nic_commit, };
