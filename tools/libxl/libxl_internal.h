...
_hidden void libxl__logv(libxl_ctx *ctx, xentoollog_level msglevel,
		int errnoval, const char *file /* may be 0 */,
		int line /* ignored if !file */, const char *func /* may be 0 */,
		const char *fmt, va_list al) __attribute__((format(printf,7,0)));

_hidden void libxl__log(libxl_ctx *ctx, xentoollog_level msglevel, int errnoval,
		const char *file /* may be 0 */, int line /* ignored if !file */,
		const char *func /* may be 0 */, const char *fmt, ...)
...
typedef void libxl__domain_create_cb(struct libxl__egc *egc,
		libxl__domain_create_state *dcs, int rc, uint32_t domid);
...
_hidden void libxl__alloc_failed(libxl_ctx *, const char *func, size_t nmemb,
		size_t size) __attribute__((noreturn));
...
typedef void libxl__ev_fd_callback(libxl__egc *egc, libxl__ev_fd *ev, int fd,
...
struct libxl__ao_abortable {
	...
	bool registered;LIBXL_LIST_ENTRY(libxl__ao_abortable)
	entry;
	/*
	 * For nested aos:
...
static inline void libxl__ao_abortable_init(libxl__ao_abortable *c) {
	c->registered = 0;
}
static inline bool libxl__ao_abortable_isregistered(
		const libxl__ao_abortable *c) {
	return c->registered;
}
...
typedef void libxl__ev_time_callback(libxl__egc *egc, libxl__ev_time *ev,
		const struct timeval *requested_abs, int rc); /* TIMEDOUT or ABORTED */
struct libxl__ev_time {
	...
	LIBXL_TAILQ_ENTRY(libxl__ev_time)
	entry;
	struct timeval abs;
	libxl__osevent_hook_nexus *nexus;
...
struct libxl__ev_evtchn {
	...
	bool waiting;LIBXL_LIST_ENTRY(libxl__ev_evtchn)
	entry;
};
...
typedef struct libxl__ev_watch_slot {
	LIBXL_SLIST_ENTRY(struct libxl__ev_watch_slot)
	empty;
...
struct libxl__ev_child {
	...
	LIBXL_LIST_ENTRY(struct libxl__ev_child)
	entry;
};
...
struct libxl__evgen_domain_death {
	uint32_t domid;
	unsigned shutdown_reported :1, death_reported :1;LIBXL_TAILQ_ENTRY(libxl_evgen_domain_death)
	entry;
...
struct libxl__evgen_disk_eject {
	...
	uint32_t domid;LIBXL_LIST_ENTRY(libxl_evgen_disk_eject)
	entry;
	libxl_ev_user user;
...
struct libxl__poller {
	 ...
	 * pollers_event.
	 */
	LIBXL_LIST_ENTRY(libxl__poller)
	entry;
	 ...
	 */
	LIBXL_LIST_ENTRY(libxl__poller)
	fds_changed_entry;bool fds_changed;
};
...
struct libxl__ctx {
	...
	LIBXL_LIST_HEAD(, libxl__poller)
	pollers_event, pollers_idle;LIBXL_LIST_HEAD(, libxl__poller)
	pollers_fds_changed;

	LIBXL_SLIST_HEAD(libxl__osevent_hook_nexi, libxl__osevent_hook_nexus)
			hook_fd_nexi_idle, hook_timeout_nexi_idle;LIBXL_LIST_HEAD(, libxl__ev_fd)
	efds;LIBXL_TAILQ_HEAD(, libxl__ev_time)
	etimes;

	libxl__ev_watch_slot *watch_slots;
	int watch_nslots, nwatches;LIBXL_SLIST_HEAD(, libxl__ev_watch_slot)
	watch_freeslots;
	...
	LIBXL_LIST_HEAD(, libxl__ev_evtchn)
	evtchns_waiting;
	libxl__ev_fd evtchn_efd;

	LIBXL_LIST_HEAD(, libxl__ao)
	aos_inprogress;

	LIBXL_TAILQ_HEAD(libxl__evgen_domain_death_list, libxl_evgen_domain_death)
			death_list /* sorted by domid */, death_reported;
	libxl__ev_xswatch death_watch;

	LIBXL_LIST_HEAD(, libxl_evgen_disk_eject)
	disk_eject_evgens;

	const libxl_childproc_hooks *childproc_hooks;
	void *childproc_user;
	int sigchld_selfpipe[2]; /* [0]==-1 means handler not installed */
	libxl__ev_fd sigchld_selfpipe_efd;LIBXL_LIST_HEAD(, libxl__ev_child)
	children;bool sigchld_user_registered;LIBXL_LIST_ENTRY(libxl_ctx)
	sigchld_users_entry;
...
struct libxl__egc {
	...
	struct libxl__gc gc;
	struct libxl__event_list occurred_for_callback;LIBXL_TAILQ_HEAD(, libxl__ao)
	aos_for_callback;LIBXL_TAILQ_HEAD(, libxl__aop_occurred)
	aops_for_callback;
};
...
struct libxl__ao {
	...
	int rc;LIBXL_LIST_HEAD(, libxl__ao_abortable)
	abortables;LIBXL_LIST_ENTRY(libxl__ao)
	inprogress_entry;
	libxl__gc gc;
	libxl_asyncop_how how;
	libxl__poller *poller;
	uint32_t domid;LIBXL_TAILQ_ENTRY(libxl__ao)
	entry_for_callback;
};
...
_hidden char *libxl__strdup(libxl__gc *gc_opt, const char *c /* may be NULL */)
		NN1;
/* duplicate at most @n bytes of string @c (similar to a gc'd strndup(3)). */
_hidden char *libxl__strndup(libxl__gc *gc_opt, const char *c /* may be NULL */,
		size_t n) NN1;
 ...
 * flags.
 */
_hidden int libxl__fd_flags_modify_save(libxl__gc *gc, int fd, int mask,
		int val, int *r_oldflags);
...
_hidden int libxl__xs_writev(libxl__gc *gc, xs_transaction_t t, const char *dir,
		char **kvs);
/* as writev but also sets the permissions on each path */
_hidden int libxl__xs_writev_perms(libxl__gc *gc, xs_transaction_t t,
		const char *dir, char *kvs[], struct xs_permissions *perms,
		unsigned int num_perms);
/* _atonce creates a transaction and writes all keys at once */
_hidden int libxl__xs_writev_atonce(libxl__gc *gc, const char *dir, char **kvs);
...
int libxl__xs_vprintf(libxl__gc *gc, xs_transaction_t t, const char *path,
		const char *fmt, va_list ap) PRINTF_ATTRIBUTE(4, 0);
int libxl__xs_printf(libxl__gc *gc, xs_transaction_t t, const char *path,
		const char *fmt, ...) PRINTF_ATTRIBUTE(4, 5);

/* On success, path will exist and will have an empty value */
int libxl__xs_mknod(libxl__gc *gc, xs_transaction_t t, const char *path,
 ...
 */
int libxl__xs_read_checked(libxl__gc *gc, xs_transaction_t t, const char *path,
...
int libxl__xs_write_checked(libxl__gc *gc, xs_transaction_t t, const char *path,
		const char *string);
...
_hidden int libxl__ev_fd_register(libxl__gc*, libxl__ev_fd *ev_out,
		libxl__ev_fd_callback*, int fd, short events /* as for poll(2) */);
_hidden int libxl__ev_fd_modify(libxl__gc*, libxl__ev_fd *ev, short events);
...
_hidden int libxl__ev_time_register_rel(libxl__ao*, libxl__ev_time *ev_out,
		libxl__ev_time_callback*, int milliseconds /* as for poll(2) */);
_hidden int libxl__ev_time_register_abs(libxl__ao*, libxl__ev_time *ev_out,
		libxl__ev_time_callback*, struct timeval);
...
static inline void libxl__ev_time_init(libxl__ev_time *ev) {
	ev->func = 0;
	libxl__ao_abortable_init(&ev->abrt);
}
...
_hidden int libxl__ev_xswatch_register(libxl__gc*, libxl__ev_xswatch *xsw_out,
		libxl__ev_xswatch_callback*, const char *path /* copied */);
...
_hidden libxl_event *libxl__event_new(libxl__egc*, libxl_event_type,
		uint32_t domid, libxl_ev_user for_user);
...
_hidden void libxl__event_disaster(libxl__egc*, const char *msg, int errnoval,
		libxl_event_type type /* may be 0 */, const char *file, int line,
		const char *func);
...
_hidden int libxl__grant_vga_iomem_permission(libxl__gc *gc,
...
typedef struct {
	...
	uint32_t console_domid;unsigned long console_mfn;

	char *saved_state;

	libxl__file_reference pv_kernel;
	libxl__file_reference pv_ramdisk;
	const char * pv_cmdline;bool pvh_enabled;
...
_hidden int libxl__domain_rename(libxl__gc *gc, uint32_t domid,
		const char *old_name, const char *new_name, xs_transaction_t trans);

_hidden int libxl__domain_resume_device_model(libxl__gc *gc, uint32_t domid);

_hidden const char *libxl__userdata_path(libxl__gc *gc, uint32_t domid,
		const char *userdata_userid, const char *wh);
...
_hidden int libxl__userdata_retrieve(libxl__gc *gc, uint32_t domid,
		const char *userdata_userid, uint8_t **data_r, int *datalen_r);
_hidden int libxl__userdata_store(libxl__gc *gc, uint32_t domid,
		const char *userdata_userid, const uint8_t *data, int datalen);
...
_hidden char * libxl__domain_pvcontrol_read(libxl__gc *gc, xs_transaction_t t,
		uint32_t domid);
...
_hidden int libxl__device_physdisk_major_minor(const char *physpath, int *major,
		int *minor);
_hidden int libxl__device_disk_dev_number(const char *virtpath, int *pdisk,
		int *ppartition);
...
_hidden int libxl__device_disk_setdefault(libxl__gc *gc,
		libxl_device_disk *disk, uint32_t domid);
...
_hidden const char *libxl__device_nic_devname(libxl__gc *gc, uint32_t domid,
 ...
 */
typedef void libxl__xswait_callback(libxl__egc *egc, libxl__xswait_state *xswa,
...
_hidden int libxl__ev_devstate_wait(libxl__ao *ao, libxl__ev_devstate *ds,
		libxl__ev_devstate_callback cb, const char *state_path, int state,
		int milliseconds);
...
typedef void libxl___domaindeathcheck_callback(libxl__egc *egc,
		libxl__domaindeathcheck*, int rc /* DESTROYED or ABORTED */);
...
_hidden int libxl__pci_topology_init(libxl__gc *gc, physdev_pci_device_t *devs,
		int num_devs);
 ...
 */
typedef void libxl__spawn_failure_cb(libxl__egc*, libxl__spawn_state*, int rc);
 ...
 */
_hidden int libxl__xenstore_child_wait_deprecated(libxl__gc *gc, uint32_t domid,
		uint32_t timeout, char *what, char *path, char *state,
		libxl__spawn_starting *spawning,
		int (*check_callback)(libxl__gc *gc, uint32_t domid, const char *state,
				void *userdata), void *check_callback_userdata);
 ...
 */
_hidden void libxl__exec(libxl__gc *gc, int stdinfd, int stdoutfd, int stderrfd,
...
_hidden int libxl__domain_make(libxl__gc *gc, libxl_domain_config *d_config,
		uint32_t *domid, xc_domain_configuration_t *xc_config);

_hidden int libxl__domain_build(libxl__gc *gc, libxl_domain_config *d_config,
...
_hidden bool libxl__query_qemu_backend(libxl__gc *gc, uint32_t domid,
		uint32_t backend_id, const char *type,
		bool def);
_hidden int libxl__dm_active(libxl__gc *gc, uint32_t domid);
_hidden int libxl__dm_check_start(libxl__gc *gc, libxl_domain_config *d_config,
		uint32_t domid);
...
_hidden int libxl__domain_device_construct_rdm(libxl__gc *gc,
		libxl_domain_config *d_config, uint64_t rdm_mem_guard,
...
_hidden int libxl__wait_for_device_model_deprecated(libxl__gc *gc,
		uint32_t domid, char *state, libxl__spawn_starting *spawning
		/* NULL allowed */,
		int (*check_callback)(libxl__gc *gc, uint32_t domid, const char *state,
				void *userdata), void *check_callback_userdata);
 ...
 */
_hidden char *libxl__blktap_devpath(libxl__gc *gc, const char *disk,
...
bool enable);
_hidden int libxl__qmp_insert_cdrom(libxl__gc *gc, int domid,
...
_hidden int libxl__qmp_x_blockdev_change(libxl__gc *gc, int domid,
		const char *parant, const char *child, const char *node);
...
/* on failure, logs */
int libxl__sendmsg_fds(libxl__gc *gc, int carrier, const void *data,
		size_t datalen, int nfds, const int fds[], const char *what);

/* Insists on receiving exactly nfds and datalen.  On failure, logs
 * and leaves *fds untouched. */
int libxl__recvmsg_fds(libxl__gc *gc, int carrier, void *databuf,
		size_t datalen, int nfds, int fds[], const char *what);
...
typedef int (*libxl__json_parse_callback)(libxl__gc *gc, libxl__json_object *o,
		void *p);
_hidden int libxl__object_from_json(libxl_ctx *ctx, const char *type,
		libxl__json_parse_callback parse, void *p, const char *s);
...
static inline flexarray_t *libxl__json_object_get_map(
...
static inline flexarray_t *libxl__json_object_get_array(
		const libxl__json_object *o) {
...
static inline long long libxl__json_object_get_integer(
		const libxl__json_object *o) {
...
_hidden int libxl__json_object_append_to(libxl__gc *gc_opt,
		libxl__json_object *obj, libxl__yajl_ctx *ctx);
...
_hidden const libxl__json_object *libxl__json_map_get(const char *key,
		const libxl__json_object *o, libxl__json_node_type expected_type);
_hidden yajl_status libxl__json_object_to_yajl_gen(libxl__gc *gc_opt,
		yajl_gen hand, libxl__json_object *param);
...
#define VLAD_AO_CREATE(ctx, domid, ao_how)                           \
    libxl__ao *ao = libxl__ao_create(ctx, domid, ao_how,        \
                               __FILE__, __LINE__, __func__);   \
    if (!ao) { libxl__ctx_unlock(ctx); return ERROR_NOMEM; }    \
    libxl__egc egc[1]; LIBXL_INIT_EGC(egc[0],ctx);              \
...
_hidden libxl__ao *libxl__ao_create(libxl_ctx*, uint32_t domid,
		const libxl_asyncop_how*, const char *file, int line, const char *func);
_hidden int libxl__ao_inprogress(libxl__ao *ao, const char *file, int line,
		const char *func); /* temporarily unlocks */
...
_hidden void libxl__device_usbdev_add(libxl__egc *egc, uint32_t domid,
		libxl_device_usbdev *usbdev, libxl__ao_device *aodev);
...
_hidden int libxl__device_from_usbctrl(libxl__gc *gc, uint32_t domid,
		libxl_device_usbctrl *usbctrl, libxl__device *device);
...
_hidden int libxl__get_hotplug_script_info(libxl__gc *gc, libxl__device *dev,
		char ***args, char ***env, libxl__device_action action, int num_exec);
...
typedef void libxl__disk_local_state_callback(libxl__egc*,
		libxl__disk_local_state*, int rc);
...
_hidden char * libxl__device_disk_find_local_path(libxl__gc *gc,
		libxl_domid guest_domid, const libxl_device_disk *disk,
		bool qdisk_direct);
...
struct libxl__datacopier_buf {
	...
	LIBXL_TAILQ_ENTRY(libxl__datacopier_buf)
	entry;
	int used;
	char buf[1000];
};
...
struct libxl__datacopier_state {
	...
	ssize_t used;LIBXL_TAILQ_HEAD(libxl__datacopier_bufs, libxl__datacopier_buf) bufs;
};
...
typedef struct libxl__save_helper_state {
	...
	void (*completion_callback)(libxl__egc *egc, void *caller_state, int rc,
			int retval, int errnoval);
	void *caller_state;
...
_hidden void libxl__conversion_helper_init(libxl__conversion_helper_state *chs);
...
typedef struct libxl__sr_record_buf {
	...
	LIBXL_STAILQ_ENTRY(struct libxl__sr_record_buf)
	entry;
...
struct libxl__stream_read_state {
	...
	int fd;bool legacy;bool back_channel;
	void (*completion_callback)(libxl__egc *egc, libxl__stream_read_state *srs,
			int rc);
	void (*checkpoint_callback)(libxl__egc *egc, libxl__stream_read_state *srs,
			int rc);
	/* Private */
	int rc;bool running;bool in_checkpoint;bool sync_teardown; /* Only used to coordinate shutdown on error path. */
	...
	libxl__sr_hdr hdr;LIBXL_STAILQ_HEAD(, libxl__sr_record_buf)
	record_queue; /* NOGC */
	enum {
		SRS_PHASE_NORMAL, SRS_PHASE_BUFFERING, SRS_PHASE_UNBUFFERING,
	} phase;bool recursion_guard;
...
typedef void libxl__domain_save_cb(libxl__egc*, libxl__domain_save_state*,
		int rc);
typedef void libxl__save_device_model_cb(libxl__egc*, libxl__domain_save_state*,
		int rc);
...
struct libxl__stream_write_state {
	...
	int fd;bool back_channel;
	void (*completion_callback)(libxl__egc *egc, libxl__stream_write_state *sws,
			int rc);
	void (*checkpoint_callback)(libxl__egc *egc, libxl__stream_write_state *sws,
			int rc);
	/* Private */
	int rc;bool running;bool in_checkpoint;bool sync_teardown; /* Only used to coordinate shutdown on error path. */
	bool in_checkpoint_state;
...
libxl__stream_write_checkpoint_state(libxl__egc *egc,
		libxl__stream_write_state *stream, libxl_sr_checkpoint_state *srcs);
...
int libxl__domain_suspend_init(libxl__egc *egc,
		libxl__domain_suspend_state *dsps, libxl_domain_type type);
...
int libxl__openptys(libxl__openpty_state *op, struct termios *termp,
		struct winsize *winp);
...
typedef void libxl__domain_destroy_cb(libxl__egc *egc,
		libxl__domain_destroy_state *dds, int rc);
...
struct libxl__destroy_domid_state {
	...
	libxl__ev_child destroyer;bool soft_reset;
};
...
struct libxl__domain_destroy_state {
	...
	int domain_finished;bool soft_reset;
};
 ...
 */
_hidden int libxl__domain_destroy(libxl__egc *egc,
		libxl__domain_destroy_state *dds);

/* Used to destroy a domain with the passed id (it doesn't check for stubs) */
_hidden int libxl__destroy_domid(libxl__egc *egc,
...
_hidden void libxl__add_disks(libxl__egc *egc, libxl__ao *ao, uint32_t domid,
		libxl_domain_config *d_config, libxl__multidev *multidev);
...
_hidden void libxl__add_usbctrls(libxl__egc *egc, libxl__ao *ao, uint32_t domid,
		libxl_domain_config *d_config, libxl__multidev *multidev);

_hidden void libxl__add_usbdevs(libxl__egc *egc, libxl__ao *ao, uint32_t domid,
		libxl_domain_config *d_config, libxl__multidev *multidev);
...
_hidden void libxl__domain_save(libxl__egc *egc, libxl__domain_save_state *dss);

/* calls libxl__xc_domain_suspend_done when done */
_hidden void libxl__xc_domain_save(libxl__egc *egc,
		libxl__domain_save_state *dss, libxl__save_helper_state *shs);
/* If rc==0 then retval is the return value from xc_domain_save
 * and errnoval is the errno value it provided.
 * If rc!=0, retval and errnoval are undefined. */
_hidden void libxl__xc_domain_save_done(libxl__egc*, void *dss_void, int rc,
		int retval, int errnoval);
...
_hidden void libxl__domain_suspend_common_switch_qemu_logdirty(int domid,
		unsigned int enable, void *data);
_hidden void libxl__domain_common_switch_qemu_logdirty(libxl__egc *egc,
		int domid, unsigned enable, libxl__logdirty_switch *lds);
...
_hidden void libxl__xc_domain_restore(libxl__egc *egc,
		libxl__domain_create_state *dcs, libxl__save_helper_state *shs, int hvm,
		int pae, int superpages);
/* If rc==0 then retval is the return value from xc_domain_save
 * and errnoval is the errno value it provided.
 * If rc!=0, retval and errnoval are undefined. */
...
static inline bool libxl__save_helper_inuse(
		const libxl__save_helper_state *shs) {
...
_hidden void libxl__remus_setup(libxl__egc *egc, libxl__remus_state *rs);
_hidden void libxl__remus_teardown(libxl__egc *egc, libxl__remus_state *rs,
		int rc);
 ...
 * it.
 */
_hidden int libxl__get_numa_candidate(libxl__gc *gc, uint32_t min_free_memkb,
		int min_cpus, int min_nodes, int max_nodes,
		const libxl_bitmap *suitable_cpumap,
		libxl__numa_candidate_cmpf numa_cmpf, libxl__numa_candidate *cndt_out,
		int *cndt_found);
...
void libxl__numa_candidate_get_nodemap(libxl__gc *gc,
		const libxl__numa_candidate *cndt, libxl_bitmap *nodemap) {
...
void libxl__numa_candidate_put_nodemap(libxl__gc *gc,
		libxl__numa_candidate *cndt, const libxl_bitmap *nodemap) {
...
int libxl__vnuma_build_vmemrange_pv_generic(libxl__gc *gc, uint32_t domid,
		libxl_domain_build_info *b_info, libxl__domain_build_state *state);
int libxl__vnuma_build_vmemrange_pv(libxl__gc *gc, uint32_t domid,
		libxl_domain_build_info *b_info, libxl__domain_build_state *state);
int libxl__vnuma_build_vmemrange_hvm(libxl__gc *gc, uint32_t domid,
		libxl_domain_build_info *b_info, libxl__domain_build_state *state,
		struct xc_dom_image *dom);
...
int libxl__cpuid_policy_list_parse_json(libxl__gc *gc,
		const libxl__json_object *o, libxl_cpuid_policy_list *p);
...
int libxl__ms_vm_genid_parse_json(libxl__gc *gc, const libxl__json_object *o,
		libxl_ms_vm_genid *p);
int libxl__int_parse_json(libxl__gc *gc, const libxl__json_object *o, void *p);
...
void libxl__update_domain_configuration(libxl__gc *gc, libxl_domain_config *dst,
		const libxl_domain_config *src);
static inline void libxl__update_config_nic(libxl__gc *gc,
		libxl_device_nic *dst, libxl_device_nic *src) {
	dst->devid = src->devid;
...
static inline void libxl__update_config_vtpm(libxl__gc *gc,
		libxl_device_vtpm *dst, libxl_device_vtpm *src) {
	dst->devid = src->devid;
...
_hidden int libxl__read_sysfs_file_contents(libxl__gc *gc, const char *filename,
