...
struct libxl__carefd {
	LIBXL_LIST_ENTRY(libxl__carefd)
	entry;
	int fd;
};
...
static LIBXL_LIST_HEAD(, libxl__carefd)
carefds = LIBXL_LIST_HEAD_INITIALIZER(carefds);
...
static LIBXL_LIST_HEAD(, libxl_ctx)
sigchld_users = LIBXL_LIST_HEAD_INITIALIZER(sigchld_users);
...
static void sigchld_selfpipe_handler(libxl__egc *egc, libxl__ev_fd *ev, int fd,
...
static void sigchld_handler(int signo) {
	...
	LIBXL_LIST_FOREACH(notify, &sigchld_users, sigchld_users_entry)
	{
		...
		if (e)
			abort(); /* errors are probably EBADF, very bad */
	}
...
static void sigchld_installhandler_core(void) /* idempotent */
{
	...
	assert(
			((void)"application must negotiate with libxl about SIGCHLD", !(sigchld_saved_action.sa_flags & SA_SIGINFO) && (sigchld_saved_action.sa_handler == SIG_DFL || sigchld_saved_action.sa_handler == SIG_IGN)));
}
...
static int childproc_reaped(libxl__egc *egc, pid_t pid, int status) {
	...
	found: childproc_reaped_ours(egc, ch, status);
	CTX_UNLOCK;
...
int libxl_childproc_reaped(libxl_ctx *ctx, pid_t pid, int status) {
	EGC_INIT(ctx);
	CTX_LOCK;
	assert(CTX->childproc_hooks->chldowner == libxl_sigchld_owner_mainloop);
	...
	for (;;) {
		...
		found: if (got == -1) {
			LIBXL__EVENT_DISASTER(egc,
					"waitpid() gave ECHILD but we have a child", ECHILD, 0);
...
static void sigchld_selfpipe_handler(libxl__egc *egc, libxl__ev_fd *ev, int fd,
	...
	if (revents & ~POLLIN) {
		...
		LIBXL__EVENT_DISASTER(egc, "unexpected poll event on SIGCHLD self pipe",
				0, 0);
	}
	...
	if (e)
		LIBXL__EVENT_DISASTER(egc, "read sigchld pipe", e, 0);
	...
	while (chldmode_ours(CTX, 0) /* in case the app changes the mode */) {
		...
		if (rc) {
			if (CTX->childproc_hooks->reaped_callback) {
				CTX_UNLOCK;
				rc = CTX->childproc_hooks->reaped_callback(pid, status,
				CTX->childproc_user);
				CTX_LOCK;
			...
			if (rc)
				libxl_report_child_exitstatus(CTX, XTL_WARN, "unknown child",
						(long) pid, status);
		}
	}
}
...
pid_t libxl__ev_child_fork(libxl__gc *gc, libxl__ev_child *ch,
	...
	out: perhaps_sigchld_notneeded(gc);
	CTX_UNLOCK;
	return rc;
}
