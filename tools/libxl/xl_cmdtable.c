...
struct cmd_spec cmd_table[] =
		{
				...
				{"server", &main_create_server, 1, 1,
						"Domain spawning server",
						"", ""},
				{"client", &main_create_client, 1, 1,
						"Domain spawning client",
						"create/destroy <ConfigFile1> <ConfigFile2> ...", ""},
				{"multiple", &main_multiple, 1, 1,
						"Create/destroy multiple domains in parallel",
						"create/destroy <ConfigFile1> <ConfigFile2> ...", "" },
				{ "config-update", &main_config_update, 1, 1,
				...
				{ "destroy", &main_destroy, 0, 1,
						"Terminate multiple domains immediately",
						"[options] <Domain1>  <Domain1> ...\n",
				...
				/*{ "multiple-destroy", &main_multiple, 0, 1,
						"Terminate multiple domains immediately",
						"[options] <Domain>\n",
						"-f                      Permit destroying domain 0, which will only succeed\n"
								"                        when run from disaggregated toolstack domain with a\n"
								"                        hardware domain distinct from domain 0." },*/
				{ "shutdown", &main_shutdown, 0, 1,
				...
				{ "pci-attach", &main_pciattach, 0, 1,
						...
						"<Domain> <BDF> [Virtual Slot]", }, { "pci-detach",
						&main_pcidetach, 0, 1,
						"Remove a domain's pass-through pci device",
						"<Domain> <BDF>", },
				{ "pci-list", &main_pcilist, 0, 0,
						"List pass-through pci devices for a domain", "<Domain>", },
				{ "pci-assignable-add", &main_pciassignable_add, 0, 1,
						"Make a device assignable for pci-passthru", "<BDF>",
						"-h                      Print this help.\n" },
				{ "pci-assignable-remove", &main_pciassignable_remove, 0, 1,
						"Remove a device from being assignable",
						"[options] <BDF>",
						"-h                      Print this help.\n"
								"-r                      Attempt to re-assign the device to the\n"
								"                        original driver" }, {
						"pci-assignable-list", &main_pciassignable_list, 0, 0,
						"List all the assignable pci devices", "", }, { "pause",
						&main_pause, 0, 1, "Pause execution of a domain",
						"<Domain>", }, { "unpause", &main_unpause, 0, 1,
						"Unpause a paused domain", "<Domain>", }, { "console",
						&main_console, 0, 0, "Attach to domain's console",
						"[options] <Domain>\n"
								"-t <type>       console type, pv or serial\n"
								"-n <number>     console number" },
				{ "vncviewer", &main_vncviewer, 0, 0,
				...
				{ "migrate-receive", &main_migrate_receive, 0, 1,
						"Restore a domain from a saved state",
						"- for internal use only", },
#endif
				{ "dump-core", &main_dump_core, 0, 1, "Core dump a domain",
						"<Domain> <filename>" }, { "cd-insert", &main_cd_insert,
						1, 1, "Insert a cdrom into a guest's cd drive",
						"<Domain> <VirtualDevice> <path>", }, { "cd-eject",
						&main_cd_eject, 1, 1,
						"Eject a cdrom from a guest's cd drive",
						"<Domain> <VirtualDevice>", },
				{ "mem-max", &main_memmax, 0, 1,
						"Set the maximum amount reservation for a domain",
						"<Domain> <MemMB['b'[bytes]|'k'[KB]|'m'[MB]|'g'[GB]|'t'[TB]]>", },
				{ "mem-set", &main_memset, 0, 1,
						"Set the current memory usage for a domain",
						"<Domain> <MemMB['b'[bytes]|'k'[KB]|'m'[MB]|'g'[GB]|'t'[TB]]>", },
				{ "button-press", &main_button_press, 0, 1,
						"Indicate an ACPI button press to the domain",
						"<Domain> <Button>",
						"<Button> may be 'power' or 'sleep'." },
				{ "vcpu-list", &main_vcpulist, 0, 0,
						"List the VCPUs for all/some domains", "[Domain, ...]", },
				{ "vcpu-pin", &main_vcpupin, 1, 1,
						"Set which CPUs a VCPU can use",
						"[option] <Domain> <VCPU|all> <Hard affinity|-|all> <Soft affinity|-|all>",
						"-f, --force        undo an override pinning done by the kernel", },
				{ "vcpu-set", &main_vcpuset, 0, 1,
						"Set the number of active VCPUs allowed for the domain",
						"[option] <Domain> <vCPUs>",
						"-i, --ignore-host  Don't limit the vCPU based on the host CPU count", },
				{ "vm-list", &main_vm_list, 0, 0,
						"List guest domains, excluding dom0, stubdoms, etc.", "", },
				{ "info", &main_info, 0, 0, "Get information about Xen host",
						"-n, --numa         List host NUMA topology information", },
				{ "sharing", &main_sharing, 0, 0,
						"Get information about page sharing", "[Domain]", },
				{ "sched-credit", &main_sched_credit, 0, 1,
				...
				{ "sched-credit2", &main_sched_credit2, 0, 1,
						"Get/set credit2 scheduler parameters",
						"[-d <Domain> [-w[=WEIGHT]]] [-p CPUPOOL]",
						"-d DOMAIN, --domain=DOMAIN     Domain to modify\n"
								"-w WEIGHT, --weight=WEIGHT     Weight (int)\n"
								"-p CPUPOOL, --cpupool=CPUPOOL  Restrict output to CPUPOOL" },
				{ "sched-rtds", &main_sched_rtds, 0, 1,
				...
				{ "domid", &main_domid, 0, 0,
						"Convert a domain name to domain id", "<DomainName>", },
				{ "domname", &main_domname, 0, 0,
						"Convert a domain id to domain name", "<DomainId>", }, {
						"rename", &main_rename, 0, 1, "Rename a domain",
						"<Domain> <NewDomainName>", },
				{ "trigger", &main_trigger, 0, 1, "Send a trigger to a domain",
						"<Domain> <nmi|reset|init|power|sleep|s3resume> [<VCPU>]", },
				{ "sysrq", &main_sysrq, 0, 1, "Send a sysrq to a domain",
						"<Domain> <letter>", }, { "debug-keys",
						&main_debug_keys, 0, 1, "Send debug keys to Xen",
						"<Keys>", },
				{ "dmesg", &main_dmesg, 0, 0, "Read and/or clear dmesg buffer",
						"[-c]",
						"  -c                        Clear dmesg buffer as well as printing it", },
				{ "top", &main_top, 0, 0,
						"Monitor a host and the domains in real time", "", },
				{ "network-attach", &main_networkattach, 1, 1,
				...
				{ "network-list", &main_networklist, 0, 0,
						"List virtual network interfaces for a domain",
						"<Domain(s)>", }, { "network-detach",
						&main_networkdetach, 0, 1,
						"Destroy a domain's virtual network device",
						"<Domain> <DevId|mac>", }, { "channel-list",
						&main_channellist, 0, 0,
						"List virtual channel devices for a domain",
						"<Domain(s)>", }, { "block-attach", &main_blockattach,
						1, 1, "Create a new virtual block device",
						"<Domain> <disk-spec-component(s)>...", },
				{ "block-list", &main_blocklist, 0, 0,
						"List virtual block devices for a domain", "<Domain(s)>", },
				{ "block-detach", &main_blockdetach, 0, 1,
						"Destroy a domain's virtual block device",
						"<Domain> <DevId>", }, { "vtpm-attach",
						&main_vtpmattach, 1, 1,
						"Create a new virtual TPM device",
						"<Domain> [uuid=<uuid>] [backend=<BackDomain>]", },
				{ "vtpm-list", &main_vtpmlist, 0, 0,
						"List virtual TPM devices for a domain", "<Domain(s)>", },
				{ "vtpm-detach", &main_vtpmdetach, 0, 1,
						"Destroy a domain's virtual TPM device",
						"<Domain> <DevId|uuid>", },
				{ "uptime", &main_uptime, 0, 0,
						"Print uptime for all/some domains", "[-s] [Domain]", },
				{ "claims", &main_claims, 0, 0,
						"List outstanding claim information about all domains",
						"", "", }, { "tmem-list", &main_tmem_list, 0, 0,
						"List tmem pools", "[-l] [<Domain>|-a]",
						"  -l                             List tmem stats", }, {
						"tmem-freeze", &main_tmem_freeze, 0, 1,
						"Freeze tmem pools", "[<Domain>|-a]",
						"  -a                             Freeze all tmem", }, {
						"tmem-thaw", &main_tmem_thaw, 0, 1, "Thaw tmem pools",
						"[<Domain>|-a]",
						"  -a                             Thaw all tmem", },
				{ "tmem-set", &main_tmem_set, 0, 1, "Change tmem settings",
						"[<Domain>|-a] [-w[=WEIGHT]|-c[=CAP]|-p[=COMPRESS]]",
						"  -a                             Operate on all tmem\n"
								"  -w WEIGHT                      Weight (int)\n"
								"  -c CAP                         Cap (int)\n"
								"  -p COMPRESS                    Compress (int)", },
				{ "tmem-shared-auth", &main_tmem_shared_auth, 0, 1,
				...
				{ "tmem-freeable", &main_tmem_freeable, 0, 0,
						"Get information about how much freeable memory (MB) is in-use by tmem",
						"", },
				{ "cpupool-create", &main_cpupoolcreate, 1, 1,
				...
				{ "cpupool-list", &main_cpupoollist, 0, 0,
						"List CPU pools on host", "[-c|--cpus] [<CPU Pool>]",
						"-c, --cpus                     Output list of CPUs used by a pool" },
				{ "cpupool-destroy", &main_cpupooldestroy, 0, 1,
						"Deactivates a CPU pool", "<CPU Pool>", }, {
						"cpupool-rename", &main_cpupoolrename, 0, 1,
						"Renames a CPU pool", "<CPU Pool> <new name>", }, {
						"cpupool-cpu-add", &main_cpupoolcpuadd, 0, 1,
						"Adds a CPU to a CPU pool",
						"<CPU Pool> <CPU nr>|node:<node nr>", }, {
						"cpupool-cpu-remove", &main_cpupoolcpuremove, 0, 1,
						"Removes a CPU from a CPU pool",
						"<CPU Pool> <CPU nr>|node:<node nr>", },
				{ "cpupool-migrate", &main_cpupoolmigrate, 0, 1,
						"Moves a domain into a CPU pool", "<Domain> <CPU Pool>", },
				{ "cpupool-numa-split", &main_cpupoolnumasplit, 0, 1,
						"Splits up the machine into one CPU pool per NUMA node",
						"", },
				{ "getenforce", &main_getenforce, 0, 0,
						"Returns the current enforcing mode of the Flask Xen security module",
						"", },
				{ "setenforce", &main_setenforce, 0, 1,
						"Sets the current enforcing mode of the Flask Xen security module",
						"<1|0|Enforcing|Permissive>", }, { "loadpolicy",
						&main_loadpolicy, 0, 1,
						"Loads a new policy int the Flask Xen security module",
						"<policy file>", },
#ifndef LIBXL_HAVE_NO_SUSPEND_RESUME
				{ "remus", &main_remus, 0, 1, "Enable Remus HA for domain",
						"[options] <Domain> [<host>]",
				...
				{ "psr-hwinfo", &main_psr_hwinfo, 0, 1,
				...
				{ "psr-cmt-attach", &main_psr_cmt_attach, 0, 1,
						"Attach Cache Monitoring Technology service to a domain",
						"<Domain>", },
				{ "psr-cmt-detach", &main_psr_cmt_detach, 0, 1,
						"Detach Cache Monitoring Technology service from a domain",
						"<Domain>", },
				{ "psr-cmt-show", &main_psr_cmt_show, 0, 1,
