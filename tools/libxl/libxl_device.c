...
char *libxl__device_frontend_path(libxl__gc *gc, libxl__device *device) {
	...
	return GCSPRINTF("%s/device/%s/%d", dom_path,
			libxl__device_kind_to_string(device->kind), device->devid);
}
...
char *libxl__device_backend_path(libxl__gc *gc, libxl__device *device) {
	...
	return GCSPRINTF("%s/backend/%s/%u/%d", dom_path,
			libxl__device_kind_to_string(device->backend_kind), device->domid,
			device->devid);
}
...
int libxl__parse_backend_path(libxl__gc *gc, const char *path,
		libxl__device *dev) {
	...
	int rc = sscanf(path, "/local/domain/%d/backend/%15[^/]/%u/%d",
			&dev->backend_domid, strkind, &dev->domid, &dev->devid);
...
int libxl__device_generic_add(libxl__gc *gc, xs_transaction_t t,
	...
	retry_transaction: if (create_transaction)
	...
	if (fents || ro_fents) {
		 ...
		 */
		if (device->kind == LIBXL__DEVICE_KIND_CONSOLE && device->devid == 0)
			xs_set_permissions(ctx->xsh, t, frontend_path, ro_frontend_perms,
		...
		if (fents)
			libxl__xs_writev_perms(gc, t, frontend_path, fents, frontend_perms,
					ARRAY_SIZE(frontend_perms));
		if (ro_fents)
...
int libxl__device_disk_set_backend(libxl__gc *gc, libxl_device_disk *disk) {
	...
	if (disk->format == LIBXL_DISK_FORMAT_EMPTY) {
		...
		memset(&a.stab, 0, sizeof(a.stab));
			...
			&& disk->backend_domid == LIBXL_TOOLSTACK_DOMID && !disk->script) {
		if (stat(disk->pdev_path, &a.stab)) {
			LOGE(ERROR, "Disk vdev=%s failed to stat: %s", disk->vdev,
					disk->pdev_path);
			return ERROR_INVAL;
		}
	}
	...
	if (disk->backend != LIBXL_DISK_BACKEND_UNKNOWN) {
		...
		if (ok)
			LOG(DEBUG, "Disk vdev=%s, using backend %s", disk->vdev,
...
int libxl__device_physdisk_major_minor(const char *physpath, int *major,
		int *minor) {
	struct stat buf;
...
static int device_virtdisk_matches(const char *virtpath, const char *devtype,
		int *index_r, int max_index, int *partition_r, int max_partition) {
	const char *p;
	char *ep;
	int tl, c;
	long pl;
...
int libxl__device_disk_dev_number(const char *virtpath, int *pdisk,
	...
	if ((sscanf(virtpath, "d%ip%i%n", &disk, &partition, &chrused) >= 2
			...
			&& partition < 256)
			|| device_virtdisk_matches(virtpath, "xvd", &disk, (1 << 20) - 1,
					&partition, 255)) {
		if (pdisk)
			*pdisk = disk;
		if (ppartition)
	...
	if (device_virtdisk_matches(virtpath, "hd", &disk, 3, &partition, 63)) {
		if (pdisk)
			*pdisk = disk;
		if (ppartition)
			*ppartition = partition;
	...
	if (device_virtdisk_matches(virtpath, "sd", &disk, 15, &partition, 15)) {
		if (pdisk)
			*pdisk = disk;
		if (ppartition)
			*ppartition = partition;
...
#define BLKIF_MINOR_EXT(dev) ((dev)&(~EXTENDED))
	...
	if (minor % nr_parts == 0)
		*ptr = 0;
		...
		snprintf(ptr, ret + 32 - ptr, "%d", minor & (nr_parts - 1));
	return ret;
#undef BUFFER_SIZE
#undef EXT_SHIFT
#undef EXTENDED
...
void libxl__multidev_prepared(libxl__egc *egc, libxl__multidev *multidev,
		int rc) {
...
static void devices_remove_callback(libxl__egc *egc, libxl__multidev *multidev,
		int rc);
...
void libxl__devices_destroy(libxl__egc *egc, libxl__devices_remove_state *drs) {
	...
	for (i = 0; i < num_kinds; i++) {
		...
		for (j = 0; j < num_dev_xsentries; j++) {
			path = GCSPRINTF("/libxl/%d/device/%s/%s/backend", domid, kinds[i],
					devs[j]);
	...
	out: libxl__multidev_prepared(egc, multidev, rc);
}
...
static void device_destroy_be_watch_cb(libxl__egc *egc,
		libxl__xswait_state *xswait, int rc, const char *data);
...
void libxl__wait_device_connection(libxl__egc *egc, libxl__ao_device *aodev) {
	...
	rc = libxl__ev_devstate_wait(ao, &aodev->backend_ds,
			device_backend_callback, state_path, XenbusStateInitWait,
...
void libxl__initiate_device_generic_remove(libxl__egc *egc,
	...
	rc = libxl__ev_devstate_wait(ao, &aodev->backend_ds,
			device_backend_callback, state_path, XenbusStateClosed,
	...
	out_success: libxl_dominfo_dispose(&info);
	return;
...
static void device_backend_callback(libxl__egc *egc, libxl__ev_devstate *ds,
	...
	if (rc == ERROR_TIMEDOUT && aodev->action == LIBXL__DEVICE_ACTION_REMOVE
			&& !aodev->force) {
...
static void device_destroy_be_watch_cb(libxl__egc *egc,
		libxl__xswait_state *xswait, int rc, const char *dir) {
...
int libxl__wait_for_device_model_deprecated(libxl__gc *gc, uint32_t domid,
		char *state, libxl__spawn_starting *spawning,
	...
	LIBXL_DEVICE_MODEL_START_TIMEOUT, "Device Model", path, state, spawning,
