...
static void run_helper(libxl__egc *egc, libxl__save_helper_state *shs,
		const char *mode_arg, int stream_fd, int back_channel_fd,
...
static void helper_stdout_readable(libxl__egc *egc, libxl__ev_fd *ev, int fd,
		short events, short revents);
static void helper_exited(libxl__egc *egc, libxl__ev_child *ch, pid_t pid,
		int status);
...
void libxl__xc_domain_restore(libxl__egc *egc, libxl__domain_create_state *dcs,
		libxl__save_helper_state *shs, int hvm, int pae, int superpages) {
	STATE_AO_GC(dcs->ao);
	...
	unsigned cbflags = libxl__srm_callout_enumcallbacks_restore(
			&shs->callbacks.restore.a);

	const unsigned long argnums[] = { domid, state->store_port,
			state->store_domid, state->console_port, state->console_domid, hvm,
			pae, superpages, cbflags, dcs->restore_params.checkpointed_stream, };
...
void libxl__xc_domain_save(libxl__egc *egc, libxl__domain_save_state *dss,
	...
	unsigned cbflags = libxl__srm_callout_enumcallbacks_save(
			&shs->callbacks.save.a);

	const unsigned long argnums[] = { dss->domid, 0, 0, dss->xcflags, dss->hvm,
	...
	NULL, 0, argnums, ARRAY_SIZE(argnums));
	return;
}
...
static void run_helper(libxl__egc *egc, libxl__save_helper_state *shs,
	...
	shs->stdin_what = GCSPRINTF(
			"domain %"PRIu32" save/restore helper" " stdin pipe", domid);
	shs->stdout_what = GCSPRINTF(
			"domain %"PRIu32" save/restore helper" " stdout pipe", domid);

	*arg++ =
			getenv("LIBXL_SAVE_HELPER") ? : LIBEXEC_BIN "/" "libxl-save-helper";
	*arg++ = mode_arg;
	...
	*arg++ = 0;
	...
	if (!pid) {
		...
		*back_channel_fd_arg = GCSPRINTF("%d", back_channel_fd);
		...
		libxl__exec(gc, libxl__carefd_fd(childs_pipes[0]),
				libxl__carefd_fd(childs_pipes[1]), -1, args[0], (char**) args,
				0);
	}
	...
	out: libxl__carefd_close(childs_pipes[0]);
	libxl__carefd_close(childs_pipes[1]);
...
void libxl__save_helper_abort(libxl__egc *egc, libxl__save_helper_state *shs) {
...
static void helper_stdout_readable(libxl__egc *egc, libxl__ev_fd *ev, int fd,
	...
	if (revents & (POLLERR | POLLPRI)) {
		LOG(ERROR, "%s signaled POLLERR|POLLPRI (%#x)", shs->stdout_what,
				revents);
		rc = ERROR_FAIL;
		out:
	...
	errnoval = libxl_read_exactly(CTX, fd, msg, msglen, shs->stdout_what,
			"ipc msg body");
	if (errnoval) {
		rc = ERROR_FAIL;
		goto out;
...
static void helper_exited(libxl__egc *egc, libxl__ev_child *ch, pid_t pid,
	...
	const char *what = GCSPRINTF("domain %"PRIu32" save/restore helper", domid);
...
static void helper_done(libxl__egc *egc, libxl__save_helper_state *shs) {
	...
	libxl__ev_fd_deregister(gc, &shs->readable);
	libxl__carefd_close(shs->pipes[0]);
	shs->pipes[0] = 0;
	libxl__carefd_close(shs->pipes[1]);
	shs->pipes[1] = 0;
	assert(!libxl__save_helper_inuse(shs));

	shs->egc = egc;
	shs->completion_callback(egc, shs->caller_state, shs->rc, shs->retval,
			shs->errnoval);
	shs->egc = 0;
}
...
void libxl__srm_callout_sendreply(int r, void *user) {
	...
	errnoval = libxl_write_exactly(CTX, libxl__carefd_fd(shs->pipes[0]), &r,
			sizeof(r), shs->stdin_what, "callback return value");
	if (errnoval)
...
void libxl__srm_callout_callback_progress(const char *context,
		const char *doing_what, unsigned long done, unsigned long total,
		void *user) {
	libxl__save_helper_state *shs = user;
	STATE_AO_GC(shs->ao);
