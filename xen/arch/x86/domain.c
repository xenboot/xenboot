...
void (*pm_idle)(void)
__read_mostly = default_idle;
void (*dead_idle)(void)
__read_mostly = default_dead_idle;
...
undo_and_fail: d->arch.is_32bit_pv = d->arch.has_32bit_shinfo = 0;
for_each_vcpu(d, v)
{
free_compat_arg_xlat(v);
...
if (!is_idle_domain(d)) {
rc = create_perdomain_mapping(d, GDT_VIRT_START(v), 1 << GDT_LDT_VCPU_SHIFT,
		d->arch.pv_domain.gdt_ldt_l1tab, NULL);
if (rc)
	goto done;

BUILD_BUG_ON(NR_VECTORS * sizeof(*v->arch.pv_vcpu.trap_ctxt) > PAGE_SIZE);
v->arch.pv_vcpu.trap_ctxt = xzalloc_array(struct trap_info, NR_VECTORS);
...
if (has_hvm_container_domain(d)) {
d->arch.hvm_domain.hap_enabled = hvm_funcs.hap_supported
...
exit_put_gfn: put_gfn(d, gfn);
exit_put_page: put_page(page);
...
if ( c(ctrlreg[0]) || c(ctrlreg[1]) || c(ctrlreg[2]) || c(ctrlreg[4])
		|| c(ctrlreg[5]) || c(ctrlreg[6]) || c(ctrlreg[7]) || c(ldt_base)
		|| c(ldt_ents) || c(user_regs.cs) || c(user_regs.ss) || c(user_regs.es)
		|| c(user_regs.ds) || c(user_regs.fs) || c(user_regs.gs) || c(kernel_ss)
		|| c(kernel_sp) || c(gdt_ents)
		|| (!compat
				&& (c.nat->gs_base_kernel || c.nat->fs_base
						|| c.nat->gs_base_user)))
	return -EINVAL;
}
...
if (is_pv_domain(d)) {
	for (i = 0; i < ARRAY_SIZE(c.cmp->trap_ctxt); ++i)
		XLAT_trap_info(v->arch.pv_vcpu.trap_ctxt + i, c.cmp->trap_ctxt + i);
}
}
...
v->arch.pv_vcpu.ctrlreg[4] =
	cr4 ? pv_guest_cr4_fixup(v, cr4) : real_cr4_to_pv_guest_cr4(
					mmu_cr4_features);
 ...
 */
if (d != current->domain && !VM_ASSIST(d, m2p_strict) && is_pv_domain(d)
...
if (!rc)
	rc = get_page_type_preemptible(cr3_page,
			!compat ? PGT_root_page_table : PGT_l3_page_table);
switch (rc) {
case -EINTR:
	rc = -ERESTART;
case -ERESTART:
	break;
case 0:
	if (!compat && !VM_ASSIST(d, m2p_strict) && !paging_mode_refcounts(d))
		fill_ro_mpt(cr3_gfn);
	break;
default:
...
if (c.nat->ctrlreg[1]) {
	...
	if (!cr3_page)
		...
		case -EINTR:
			rc = -ERESTART;
		case -ERESTART:
			v->arch.old_guest_table = pagetable_get_page(v->arch.guest_table);
			v->arch.guest_table = pagetable_null();
			break;
		default:
			if (cr3_page == current->arch.old_guest_table)
...
pvh_skip_pv_stuff: if (v->vcpu_id == 0)
...
if (ctx->cpu_regs.x86_32.pad1 != 0 || ctx->cpu_regs.x86_32.pad2[0] != 0
...
static void load_segments(struct vcpu *n) {
	 ...
	 */
	if (unlikely((dirty_segment_mask & (DIRTY_FS | DIRTY_FS_BASE)) | uregs->fs))
	 ...
	 */
	if (unlikely(
			(dirty_segment_mask & (DIRTY_GS | DIRTY_GS_BASE_USER)) | uregs->gs)) {
	...
	if (unlikely(!all_segs_okay)) {
		...
		if (is_pv_32bit_vcpu(n)) {
			...
			if (ret | put_user(eflags, esp - 1) | put_user(cs_and_mask, esp - 2)
		...
		if (put_user(regs->ss, rsp - 1) | put_user(regs->rsp, rsp - 2)
				| put_user(rflags, rsp - 3) | put_user(cs_and_mask, rsp - 4)
				| put_user(regs->rip, rsp - 5) | put_user(uregs->gs, rsp - 6)
				| put_user(uregs->fs, rsp - 7) | put_user(uregs->es, rsp - 8)
				| put_user(uregs->ds, rsp - 9) | put_user(regs->r11, rsp - 10)
...
static void __context_switch(void) {
	...
	gdt = !is_pv_32bit_domain(nd) ?
			per_cpu(gdt_table, cpu) : per_cpu(compat_gdt_table, cpu);
	if (need_full_gdt(nd)) {
...
void context_switch(struct vcpu *prev, struct vcpu *next) {
	...
	if (unlikely(
			!cpumask_test_cpu(cpu, &dirty_mask) && !cpumask_empty(&dirty_mask))) {
	...
	if ((per_cpu(curr_vcpu, cpu) == next)
		...
		if (is_pv_domain(nextd)
				&& (is_idle_domain(prevd) || has_hvm_container_domain(prevd)
						|| is_pv_32bit_domain(prevd)
								!= is_pv_32bit_domain(nextd))) {
...
unsigned long hypercall_create_continuation(unsigned int op, const char *format,
	...
	if (mcs->flags & MCSF_in_multicall) {
		...
		/*subtract 2 from the guests EIP, causing it to re-execute
		 * the int 0x82/sysenter/syscall instruction when next scheduled*/
		if (is_pv_vcpu(curr))
			regs->eip -= 2; /* re-execute 'syscall' / 'int $xx' */
		else
			curr->arch.hvm_vcpu.hcall_preempted = 1;

		/*writes the arguments of this hypercall to guest user registers*/
		if (is_pv_vcpu(curr) ?
				!is_pv_32bit_vcpu(curr) : curr->arch.hvm_vcpu.hcall_64bit) {
			for (i = 0; *p != '\0'; i++) {
				arg = next_arg(p, args);
				switch (i) {
				case 0:
...
void domain_cpuid(struct domain *d, unsigned int input, unsigned int sub_input,
...
void vcpu_mark_events_pending(struct vcpu *v) {
	int already_pending = test_and_set_bit(0,
