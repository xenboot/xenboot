...
#ifdef CONFIG_NUMA_LOCAL_SCRUB
#include <xen/local_scrub.h>
#endif /* CONFIG_NUMA_LOCAL_SCRUB */
...
static void __init bootmem_region_add(unsigned long s, unsigned long e) {
	...
	BOOT_BUG_ON(
			nr_bootmem_regions == (PAGE_SIZE / sizeof(struct bootmem_region)));
...
unsigned long __init alloc_boot_pages(unsigned long nr_pfns,
...
int domain_set_outstanding_pages(struct domain *d, unsigned long pages) {
	...
	//~ printk("total_avail_pages: %ld\n", total_avail_pages);
	//~ printk("outstanding_claims: %ld\n", outstanding_claims);
	//~ printk("claim: %lu\n", claim);
	//~ printk("avail_pages: %lu\n", avail_pages);
...
static unsigned long init_node_heap(int node, unsigned long mfn,
	...
	unsigned long needed = (sizeof(**_heap) + sizeof(**avail) * NR_ZONES +
	...
	} else if (nr >= needed && (mfn + needed) <= (virt_to_mfn(eva - 1) + 1)
			&& (!xenheap_bits
...
static void check_low_mem_virq(void) {
	unsigned long avail_pages = total_avail_pages + tmem_freeable_pages()
			- outstanding_claims;
...
static struct page_info *alloc_heap_pages(unsigned int zone_lo,
		unsigned int zone_hi, unsigned int order, unsigned int memflags,
		struct domain *d) {
...
static void free_heap_pages(struct page_info *pg, unsigned int order) {
	...
	//printk("free_heap_pages\n");
	avail[node][zone] += 1 << order;
	total_avail_pages += 1 << order;

	if (tmem_enabled())
		midsize_alloc_zone_pages = max(midsize_alloc_zone_pages,
				total_avail_pages / MIDSIZE_ALLOC_FRAC);
...
int offline_page(unsigned long mfn, int broken, uint32_t *status) {
	...
	if (!mfn_valid(mfn)) {
		dprintk(XENLOG_WARNING, "try to offline page out of range %lx\n", mfn);
		return -EINVAL;
	}
	...
	*status = 0;
	...
	if (page_state_is(pg, offlined)) {
		...
		*status =
				broken ?
						PG_OFFLINE_OFFLINED | PG_OFFLINE_BROKEN :
						PG_OFFLINE_OFFLINED;
		return 0;
	}
...
int assign_pages(struct domain *d, struct page_info *pg, unsigned int order,
	...
	if (!(memflags & MEMF_no_refcount)) {
		if (unlikely((d->tot_pages + (1 << order)) > d->max_pages)) {
			if (!tmem_enabled() || order != 0 || d->tot_pages != d->max_pages)
				gprintk(XENLOG_INFO,
						"Over-allocation for domain %u: " "%u > %u\n",
...
#ifdef CONFIG_NUMA_LOCAL_SCRUB
PAGE_LIST_HEAD(scrub_list);
struct spinlock scrub_list_lock = SPIN_LOCK_UNLOCKED;
#endif /* CONFIG_NUMA_LOCAL_SCRUB */
...
#ifdef CONFIG_NO_SCRUB
        scrub = 0;
#endif /* CONFIG_NO_SCRUB */

#ifdef CONFIG_NUMA_LOCAL_SCRUB
        if ( unlikely(scrub) )
        {
            int cpu = 0;
            int scrubbing_done = 0;
            static int count = 0;
            count++;

            spin_lock(&scrub_list_lock);
            for ( i = 0; i < (1 << order); i++ )
                page_list_add(&pg[i], &scrub_list);
            spin_unlock(&scrub_list_lock);
            
            if(count % 4096 == 0)
            {
                cpu =  d ? cpumask_first(d->vcpu[0]->cpu_hard_affinity) : 0;
                on_selected_cpus(cpumask_of(cpu), scrub_one_page_wrap, 0, 1);

                while(!scrubbing_done)
                {
                    spin_lock(&scrub_list_lock);
                    scrubbing_done = page_list_empty(&scrub_list);
                    spin_unlock(&scrub_list_lock);
                }

                //  while(!page_list_empty(&scrub_list));
...
void scrub_one_page_wrap(void *param)
{
    ...
    while(1)
    {
        pg = page_list_remove_head(&scrub_list);
...
unsigned long avail_domheap_pages_region(unsigned int node,
		unsigned int min_width, unsigned int max_width) {
	int zone_lo, zone_hi;
