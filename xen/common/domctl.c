...
static int xenctl_bitmap_to_bitmap(unsigned long *bitmap,
		const struct xenctl_bitmap *xenctl_bitmap, unsigned int nbits) {
...
static unsigned int default_vcpu0_location(cpumask_t *online) {
	...
	if (cnt) {
		...
		for_each_domain (d)
		for_each_vcpu(d, v)
		if (!(v->pause_flags & VPF_down) && ((cpu = v->processor) < nr_cpus))
			cnt[cpu]++;
...
bool_t domctl_lock_acquire(void) {
	 ...
	 * against other non-domctl routines which try to pause the vCPUs.
	 */
...
static struct vnuma_info *vnuma_alloc(unsigned int nr_vnodes,
		unsigned int nr_ranges, unsigned int nr_vcpus) {
...
long do_domctl(XEN_GUEST_HANDLE_PARAM(xen_domctl_t) u_domctl) {
	...
	if (!domctl_lock_acquire()) {
		if (d)
			rcu_unlock_domain(d);
		/*re-execute this hypercall in the next time slice*/
	...
	case XEN_DOMCTL_createdomain: {
		...
		if ((op->u.createdomain.flags
				& ~(XEN_DOMCTL_CDF_hvm_guest | XEN_DOMCTL_CDF_pvh_guest
						| XEN_DOMCTL_CDF_hap | XEN_DOMCTL_CDF_s3_integrity
						| XEN_DOMCTL_CDF_oos_off | XEN_DOMCTL_CDF_xs_domain)))
			break;
	...
	case XEN_DOMCTL_max_vcpus: {
		...
		maxvcpu_out_novcpulock: domain_unpause(d);
		break;
	}
    ...
    case XEN_DOMCTL_destroydomain:
    {
        ...
        {
            /* scrub the any leftover in the list */
            int cpu =  d ? cpumask_first(d->vcpu[0]->cpu_hard_affinity) : 0;
	...
	case XEN_DOMCTL_getvcpuaffinity: {
		...
		if (op->cmd == XEN_DOMCTL_setvcpuaffinity) {
			...
			setvcpuaffinity_out: free_cpumask_var(new_affinity);
			free_cpumask_var(old_affinity);
		} else {
			if (vcpuaff->flags & XEN_VCPUAFFINITY_HARD)
				ret = cpumask_to_xenctl_bitmap(&vcpuaff->cpumap_hard,
	...
	case XEN_DOMCTL_getvcpucontext: {
		...
		getvcpucontext_out: xfree(c.nat);
		break;
	}
	...
	case XEN_DOMCTL_set_max_evtchn:
		d->max_evtchn_port = min_t(unsigned int, op->u.set_max_evtchn.max_port,
				INT_MAX);
		break;
	...
	domctl_out_unlock_domonly: if (d)
		rcu_unlock_domain(d);
